...
- **Mirrors Claude's memory structure.** Claude's memory has types (user, feedback, project, reference). This playbook is similar but simpler: categories by purpose. The frontmatter is light; conventions are loose. It's the human-readable mirro