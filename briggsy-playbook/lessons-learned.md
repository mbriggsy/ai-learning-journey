...
**Incident:** Deepening + doc-reviewing ai-journey-stats Phase 5 (the per-project detail page). The plan was thorough — paint-by-numbers recipe, every section spec'd, null-degrade rules per field. The 7-persona document-review caught a through-line none of the individual section specs revealed: **the whole plan was designed and verified against BURNED — the data-RICH project where every field is populated — and never against the project it would actually ship FIRST: the `project-metrics` tool itself, on a clean deploy box.**
...
**Fixes integrated:** a content floor (≤3 inventory items render as a centered row, not a gapped grid); editorial (`oneLiner`+`description`) promoted to a HARD dependency for every detail page so the story always renders even when the numbers are sparse; and a verify-gate scenario that tests the combined-null case AND the actual `project-metrics` meta project by name.
...
3. **Identify the first-shipped / most-clicked instance explicitly and name it in the verify gate.** "The flagship `project-metrics` meta project on a clean deploy" is a sharp, checkable target; "various edge cases" is not.
...
**Memorialized in:** `projects/ai-journey-stats/docs/plans/phase-5-detail.md` (Decision 12 — content floor + editorial-as-hard-dependency; verify gates 4a/11a), `projects/ai-journey-stats/TODO.md` (Landmines), and this entry.

---

## 2026-05-24 — Inherited "desktop showcase" framing in ai-journey-stats TODO

**Incident:** Mid-planning on ai-journey-stats (the public showcase site for the `project-metrics` CLI). I asked Claude to revise the TODO based on the WHAT-interview decisions. Claude did the revision well — applied all the bar revisions, expanded Phase 0 with the new metrics, added editorial schema — but left the line *"Mobile-first deep optimization beyond 'doesn't break' (this is a desktop showcase)"* sitting in the out-of-scope section. The phrasing was inherited from the pre-WHAT version of the TODO.
...
- TODO updates in `projects/ai-journey-stats/TODO.md`
