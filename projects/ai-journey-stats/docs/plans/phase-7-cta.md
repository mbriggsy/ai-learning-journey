...
> **SUPERSEDED 2026-05-27 (Briggsy).** The three-figure magnitude stack (Decision 2) is REPLACED by the §11 sign-off at **display weight** — *"Claude wrote all of it. Briggsy directed — and answered a question or two."* (lead clause bold/primary, second clause softer/secondary). Rationale: the hero already owns the magnitude, so re-printing projects/tokens/lines at the close read as padding (Phase 9 cold-read + stranger-proxy). The close is now **static text** — no data wire, no null-degrade; `deriveCloseModel` + `src/lib/close-model.ts` + its test were removed with the stats. The reveal machinery (Decision 5: below-fold ScrollTrigger + refreshInit/refresh y-restore + post-settle rect-check) is UNCHANGED. The line is §11-compliant (a warm note, not a who-wrote-what scoreboard) and also stays as the About closing line. ideation §4/§11 + editorial-spine updated to match. Everything below about the stack's *composition/data* is HISTORY.
