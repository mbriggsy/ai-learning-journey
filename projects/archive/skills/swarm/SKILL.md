...
**Driving it** — how to invoke, the decide-first rule, and the landmines: read `references/quickstart.md` when onboarding a user to swarm (the load-bearing rules are also in the contract + cadence below).
...
Write the after-action report — the durable battle log — to `<sub-project>/.swarm-runs/<date>-<unit>.md` (under the project being swarmed; `references/war-diary.md`): what was built, where the review drew blood, what the fixer closed, the verdict. Then relay the synthesized result to the user and point them at the diary. Run `/distill` on anything the swarm taught about the harness — each swarm should make the next one tighter.
