...
- **§3.5 Phrasing!** — Archer's signature catchphrase is core BURNED tone DNA. **Trigger** = the line preceding `…Phrasing.` must read SIMULTANEOUSLY as benign briefing-context AND a sexual double entendre (cold-listen "that's what she said" test). Without the entendre setup the tag is unearned — never tag a non-entendre line. **Cadence** = abundance, not restraint — seed entendre-earned beats generously across ✅ surfaces (random flavor pools, AnnouncementFeed, DramaOverlay, Lobby/idle text). Avoid ❌ surfaces (errors, repeat-view text, rule text). Spec §3.5 carries the trigger rule + the shipped-beats catalog; the rotating wire-report pool lives in `BURNED_PHRASING_POOL` in `src/client/shared/DramaOverlay.tsx`.
