...
- **Hand-reorder layoutId — investigated, no test shipped.** When a card stages out of `Hand.tsx`, the remaining cards' `layout="position"` produces a near-instant jump (one rAF tick) rather than a smooth animation — likely because the slot's explicit `animate={{ transform: 'translateX(0px) scale(1)' }}` fights Framer's layout-derived transform on the same property. User-facing: card flies to staging cleanly, remaining cards snap to fill the gap. Building a runtime gate for an animation that doesn't visibly run would be theater. If a future session wants to revisit, fix the `transform` conflict first (split into separate `x`/`scale` props or remove the explicit transform during reorder), then build the gate against the now-real cinematic.
