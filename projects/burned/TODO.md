...
_No live prescriptions on the BURNED product. **Origin-trailer plan state (2026-05-17 mid-session): doc-review sweep COMPLETE across all 8 phase plans (0/1/2/3/4/5/6/7). Phase 7 doc-review absorbed THIS COMMIT — 7-persona pass (coherence 5 / feasibility 5 / product-lens 10 / design-lens 5 / security-lens 6 / scope-guardian 7 / adversarial 10 = ~48 raw findings, ~38 unique absorbed after dedup across all severities). 2539→3279 lines Phase 7 (1.29× growth — modest, deepening already absorbed many fixes); roadmap +19 lines net. 8 strategic CALLs locked: CALL-1 (caption audience-fit + Variation A-alt — pre-doc-review locked Variation A "party game" caption silently; engineering-peer audience locked in project context, but Variation A leads with party-game framing and buries the agentic-SDLC claim at Post 2b T+2-3h after 50-70% of impressions land in first hour; A-alt drafted as 280-char engineering-peer-lead variant with no link, Briggsy locks audience-fit choice deliberately at sign-off); CALL-2 (pin lifecycle collapse — pre-doc-review Unit 7.7 installed D+180/D+270/D+360 ladder + cross-session TODO.md auto-renewing row for a no-deadline portfolio piece; collapsed to new-project-trigger PRIMARY + D+360 one-shot reconsideration FALLBACK, single TODO.md row that self-deletes); CALL-3 (cutdown frame-0 audience-aware Option ranking — pre-doc-review Option C default opens on cold-open character beat, mismatched with engineering-peer "product-in-motion" autoplay-entry pattern; ranked rule added to Unit 7.1 Step 4, with explicit identity-bet acknowledgment if character-first cold-open kept); CALL-4 (README cutdown-hero trade-off explicit — kept option (b) drag-drop cutdown lock from deepening but added explicit "we chose this because" framing acknowledging engineering-Twitter visitors may have just watched the cutdown on X); CALL-5 (cut "...Phrasing." caption tag per project-burned-sterling-coded-voice.md — voice rule is Sterling-CODED cadence mimicry, NEVER Benjamin-cloned identity; catchphrase verbatim on a launch tweet is costume, not cadence; PRODUCT-SPECIFICATION §3.5 ✅ surfaces explicitly do NOT list distribution copy); CALL-6 (decouple character-cap from audience-fit message — pre-doc-review variant matrix was account-state-as-content-gate; matrix restructured as (account-state) × (audience-fit) with 280-char engineering-peer-lead Variation A-alt that fits free tier and link-demotion-free regime); CALL-7 (D+30 metrics method-signals-first + ADR #27 falsification test — for engineering-proving-ground project the success criterion is "did method claim register" not raw impressions; metrics-log reordered to lead with method-signal mentions + UMB cross-reference recognition + "how did you build this" DM count; ADR #27 3-beat burst hypothesis given a falsification check at D+30 — Post 1 >85% of thread impressions = weakly disconfirmed, Post 2/2b each >10% = confirmed); CALL-8 (minimum-viable-distribution vs pattern-investment honesty section — names which units are load-bearing for engineering-proving-ground goal vs which are pattern-investment-for-future-projects; full machinery kept because marginal cost is small and propagation plausible, but bet named explicitly so D+30 + first-future-project trigger has an audit row). P0 mechanical fixes: raw FFmpeg `-x264-preset` flag UNRECOGNIZED — `ffmpeg ... -c:v libx264 -x264-preset slow` returns `Unrecognized option 'x264-preset'` and exits non-zero (verified 2026-05-17 against installed FFmpeg 8.1); flag is Remotion CLI spelling only, raw FFmpeg uses libx264 encoder option `-preset slow` (single dash, no `x264-` prefix); fix applied across Unit 7.1 Approach 2a + 2b + Step 6 9:16 render + D-7B + cutdown-eval.md template + Risks + Open Questions resolved; ADR #19 amended with cross-tool note (Remotion CLI: `--x264-preset slow`; raw FFmpeg: `-preset slow`). Caption "one in the basement" → "one on the research budget" matching Phase 1 line **1086** LOCKED narration across captions / verifier hardcoded constant / stat enumeration / roadmap §1 / §6 row C7 — the prior Phase 1 "basement" framing was Phase 1 fiction explicitly retracted at Phase 1 lines 1100-1113 during Phase 1 doc-review (ActRoster.tsx:153-158 canonical: Otto "busy with the research budget"); Phase 7 pre-doc-review inherited the retracted text and propagated it forward. Thumbnail default frame 2790 → frame 1950 PRIMARY (with 1860/1425/2790 fallback ladder) per Phase 6 Unit 6.2 Step 3 **lines 1232-1240** — the pre-doc-review citation of "Phase 6 line 2253" was a mobile-crop-audit table row, not the selection rule; mid-process feed-stoppers preferred over logo-closure per Phase 6 Adversarial Attack 14 + Product-lens F3 (logo-on-desk is what engineering Twitter scrolls past). P1 wiring gaps closed: verify-phase-7-entry.ts + verify-briggsy-sentinels.ts script creation contracted in Unit 7.0 Step 0 (pre-doc-review plan invoked them without assigning creation, citing them as "existing" — package.json shipped only `verify:bundle`); release-notes.md authoring contracted in Unit 7.1b Step 0 + pre-flight `gh auth status` + `gh repo view` (pre-doc-review's `--notes-file` would have failed on first run); briggsy-review-7.1b.signoff + briggsy-review-7.2.signoff added to D-7I + Unit 7.6 sentinel array (drift between creation site and verify site); user-attachments URL post-drag-drop capture contracted as Step 2 substep 9 (`<UUID-cutdown>` placeholder otherwise unreproducible in committed artifacts per adversarial A3); Approach 2b (per-segment + concat-demuxer) promoted to Phase 7 cutdown DEFAULT per adversarial audio-seam drift concern (filter_complex `atrim` is sample-accurate but AAC packet boundary is 21.33ms — accumulated drift across multi-segment concat can produce audio-lead at any seam, exactly the ADR #20 R3 failure mode); 2a kept as documented fallback only. Per-seam AV-sync verification added to Step 5 (pre-doc-review was per-cue only). README `## Trailer` placement re-anchored to "AFTER `## Play` BEFORE `## Project Map`" — pre-doc-review audit missed the `## Play` section entirely. Frame-0 audit gains greyscale color-blind-safe check per design-lens D5 (Briggsy is color-blind; if frame 0 reads only via hue it fails autoplay-entry test). Step 6 X composer rendering verification extended to all 4 posts (Post 1 + Post 2 + Post 2b + Post 3) — pre-doc-review ran only on locked Post 1 variant; Post 3 is a permanently-pinned tweet whose em-dash autoformat collapse would bake for the life of the pin. GitHub plan state added to Entry Gate alongside X account state per adversarial A8 — ADR #29 user-attachments cap branches by plan (Free 10 MiB / Pro+ 100 MiB; cutdown at ~25-40 MB fits Pro+ inline but NOT Free); attachment cap docs cited as MiB not MB with primary-source date-pin per adversarial A9. Repo-publicity check added to Entry Gate per security-lens S5 (account-state.md commits to git history; if repo public, surface briggsy007@gmail.com identity + tiers together — gitignore branch). Verifier improvements: `node:fs.globSync` option is `exclude` not `ignore` (silently dropped — F4); TS import paths drop `.js` extension matching project tsx convention (F5); `source_git_sha` capture via `execFileSync('git', ['rev-parse', 'HEAD'])` not placeholder comment (S2); `OPERATIVES_OFF_BOOKS = 1` promoted to `src/shared/narrative-stats.ts` constant imported by verifier (A7 — pre-doc-review hardcoded inline defeats the verifier's stated purpose); Otto-doesn't-ship invariant assertion added; phantom stat-table rows (`phases_in_plan`, `phase_files_drafted`, `act_count_in_htp`, `card_types`) removed per SG5/SG6 (framework-ahead-of-need; enumerate only stats that ship in copy). Portfolio-site design tokens trimmed to "defer to Briggsy" per SG7 (configuration for hypothetical surface that doesn't exist). UMB cross-reference dropped from README + Variation D per product-lens P2 / design-lens D4 (UMB v3 has no documented reception; engineering-mystique framing self-supporting). Pin Post 1 timing D+7 → D+0 evening per product-lens P6 + CALL-2 (D+7 left flagship unpinned through 50-70% impression window the rest of plan optimizes for). Cross-phase amendments LANDED same commit (roadmap §1 + §6 row C7 "basement"→"research budget" + ADR #19 cross-tool Remotion CLI vs raw FFmpeg note + ADR #26 narrative-stats import + Otto-doesn't-ship invariant). Phase 6 amendment-status verification (per adversarial A10): confirmed all 5 cross-phase amendments from Phase 7 deepening actually landed in Phase 6 body (lines 1232-1240 thumbnail rule + lines 1244-1254 JPEG derivative + lines 2861-2867 contract tightening + lines 2869-2886 composed-not-mid-motion template + line 2888 file-size template). Phase 7 frontmatter `reviewed: 2026-05-17`. Doc-review sweep across all 8 phase plans COMPLETE; next: Phase 0 execution begins after Briggsy reviews deepened+reviewed plan set.**_
...
- _[`phase-7-distribution.md`](docs/plans/origin-trailer/phase-7-distribution.md) ✅ **DEEPENED 2026-05-17 + DOCUMENT-REVIEW APPLIED 2026-05-17** — 9 units (deepened: ~50 amendments + 4 NEW units 7.0 stat-verification gate / 7.1b Release asset / 7.6 pre-post verify gate / 7.7 pin lifecycle); ADRs #24-29 added during deepening. ~48 raw doc-review findings → ~38 unique absorbed across 7 CE personas (coherence 5 / feasibility 5 / product-lens 10 / design-lens 5 / security-lens 6 / scope-guardian 7 / adversarial 10). 2539→3279 lines (1.29× growth). 8 strategic CALLs locked (CALL-1 caption audience-fit + Variation A-alt / CALL-2 pin lifecycle collapse / CALL-3 cutdown frame-0 audience-aware Option ranking / CALL-4 README cutdown-hero trade-off explicit / CALL-5 cut "...Phrasing." per Sterling-CODED / CALL-6 decouple character-cap from audience-fit / CALL-7 D+30 metrics method-signals-first + ADR #27 falsification / CALL-8 minimum-viable-distribution honesty section). P0 fixes: raw FFmpeg `-x264-preset` → `-preset` (Remotion CLI vs raw FFmpeg cross-tool note added to ADR #19; verified non-zero exit on FFmpeg 8.1); caption "in the basement" → "on the research budget" matching Phase 1 line 1086 LOCKED narration (the prior Phase 1 "basement" framing was Phase 1 fiction retracted at Phase 1 doc-review — Phase 7 pre-doc-review inherited the retracted text); thumbnail default frame 2790 → frame 1950 PRIMARY (per Phase 6 Unit 6.2 Step 3 lines 1232-1240 — pre-doc-review citation "line 2253" was a crop-audit table row, not the rule). Wiring gaps: verify-phase-7-entry.ts + verify-briggsy-sentinels.ts + release-notes.md creation contracted; briggsy-review-7.1b + 7.2 sentinels added to D-7I + verify-cutdown-ready.ts. Approach 2b promoted to cutdown DEFAULT per audio-seam drift; 2a kept as fallback. GitHub plan state added to Entry Gate (ADR #29 attachment caps branch by plan). Phase 6 cross-phase amendment status verified-landed. Phase 7 frontmatter `reviewed: 2026-05-17`._
