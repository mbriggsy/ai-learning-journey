...
_No live prescriptions on the BURNED product. **Origin-trailer plan state (2026-05-17 mid-session): Phases 0/1/2/3/4/5 doc-review applied; Phases 6/7 pending. Phase 5 doc-review absorbed THIS COMMIT — 7-persona pass (coherence 18 / feasibility 30 / product-lens 25 / design-lens 23 / security-lens 12 / scope-guardian 27 / adversarial 28 = ~80 unique findings, ~23 P0s). 2283→3599 lines (1.58× growth). 7 strategic CALLs locked (Mechanism A spike PROMOTED to Unit 5.0 parallel-prereq; R13 split into R13a legitimacy + R13b aliveness; agent-built identity thesis-defense; Mechanism B = physical-camera-with-OBS-VCD not Display-Capture-alone; Approach III LOCAL-DEV-ONLY; Harry as outside-viewer NOT physical participant; URL strategy mechanism-coupled). P0 technical fixes: BURNED-canon card names in dev:stack invocations (pre-deepening EK names absent from CARD_DEFS → INVALID_CARD_TYPE), corrected CLI signature (space-separated argv + room positional), seed-after-game-start sequence (lobby → Cleared Hot → playing → dev:stack), godClient.send() replaced with child-process spawn (was fictional API), parallel `scripts/playtest/trailer-capture.ts` replacing --trailer-capture CLI flag (parseArgv rejects), `verify-briggsy-sentinels` scope extension cross-phase amendment to Phase 4, HEAD_TRIM_FRAMES from env var with NaN-assertion (was hardcoded "EXAMPLE" 318 → silent wrong-trim), framerate-aware head-trim math for Mechanism A WebM VFR, atomicSwap EXDEV/EEXIST handling, verify-gameplay-clip extended with pix_fmt + r_frame_rate + field_order. Take selection ladder: rubric-floor → 24h cool-off → random-order top-3 → §2 Archer gate → 3-question fluency gate → Harry outside-eye blind viewer. Phase 5 frontmatter `reviewed: 2026-05-17`. Phase 1 cross-phase amendments LANDED same commit (retired gameplay-markers.json + startFrom/endAt contract per F13; cue-table disambiguation per coherence F1 + adversarial F28; codename-aliases subsume scrub field). Phase 1: 3560→3601 lines (1.01× growth — surgical amendments only). PHASE-5-PREFLIGHT.md ceremony dropped (folded into capture-log.md mechanism-lock header). PHASE-5-EXIT.md template trimmed to 4 facts + Phase 6 read-points. Doc-review sweep continues sequentially through Phase 6/7.**_
...
- _[`phase-6-final-render-qa.md`](docs/plans/origin-trailer/phase-6-final-render-qa.md) ✅ **DEEPENED 2026-05-17** — 9 units now (NEW 6.0 verify-script + atomic-swap + tester pre-recruitment + NEW 6.8 PHASE-6-EXIT + cutdown-frame-list; 6.1-6.7 with promote-or-rerender decision + ADR #19 canonical encoding + Phase 4 GAMEPLAY_CLIP_SOURCE acknowledgment + cross-browser + X staging + operational §2 rubric + contact-sheet + bar-raise composite + axis 3 necessary-not-sufficient threshold + manifest-driven A/V sync + ADR #20 asymmetric tolerance + 9:16 vertical audit per ADR #23 + N=6 decode panel + UMB control + Q1/Q2 + sentinel ceremony per ADR #22 + verdict-first qa-report + Briggsy end-to-end watch); ~45 amendments via same 8-agent shape (best-practices / framework-docs / repo-research / adversarial / scope-guardian / coherence / feasibility / design-lens) + /brief; 1293→2413 lines (1.87× growth)_
- _[`phase-7-distribution.md`](docs/plans/origin-trailer/phase-7-distribution.md) — 5 units (X-native cutdown + portfolio embed + post copy + calendar + metrics tracking) — **next deepening target**_
