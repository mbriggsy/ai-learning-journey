...
_No live prescriptions on the BURNED product. **Origin-trailer plan state (2026-05-17): Phase 0 DEEPENED (`b9617d9d`); Phase 1 DEEPENED (`43d44ef4`, 60+ amendments, 1862→2728 lines); Phase 2 DEEPENED (`e56e69e5`, 35 amendments across 4 tiers, 1929→4064 lines, +3 new units 2.0/2.X/2.Y); Phase 3 DEEPENED (`e565b464`, 69 amendments across 5 tiers, 1737→4379 lines, 2.52× growth, +1 new unit 3.0 vocabulary vendoring); Phase 4 DEEPENED + DOCUMENT-REVIEW APPLIED (this session, 28 deepening amendments + 11 document-review amendments across TIER 1/2/3, 2467→~3900 lines, +2 new units 4.0 font spike + 4.0a UMB v3 component triage + 2 new sub-steps 4.6/1c S05HeadFadeFromBlack + 4.9/3a sentinel git-author + 4.9/5b MP4-encoded motion-shape spec); Phases 5–7 drafted + committed (`de20650b`); Phase 5 deepening is the next live work item.**_

- _[`roadmap.md`](docs/plans/origin-trailer/roadmap.md) — 18 ADRs now (NEW ADR #16 composition-level audio + ADR #17 Audio-import discipline + ADR #18 font load strategy placeholder added 2026-05-17 per Phase 4 deepening), R1–R15 trace, 3-axis bar-raise criteria, ~95s/2850-frame target_
...
- _[`phase-5-gameplay-capture.md`](docs/plans/origin-trailer/phase-5-gameplay-capture.md) — 6 units (mechanism evaluation + shot list + harness + take selection + post-process + Phase 4 re-render) — **next deepening target**_
...
_1. **Commit the Phase 4 deepening + document-review package** as a single commit (matches Phase 0/1/2/3 deepening commit pattern). Suggested commit subject: `docs(burned): Phase 4 deepening pass + document-review pass + roadmap ADRs #16/#17/#18`. Includes `phase-4-remotion-composite.md` (deepening + 11 amendments), `roadmap.md` (3 new ADRs), `TODO.md` (this updated state). EXCLUDES quarantined deploy-migration files (board.html / player.html / _headers / ActRemote.tsx / room.ts / deploy-burned.yml — separate deployment commit when ready)._
_2. **`/deepen-plan` on `phase-5-gameplay-capture.md`.** Per `feedback-phase-plan-drafting-workflow.md`. Sequential, one phase per session. Phase 5 must absorb: (a) 4 ORIGINAL cross-phase deps (gameplay path `public/trailer/gameplay.mp4` / audio-stripping `ffmpeg -an` / atomic-swap + ffprobe gate / placeholder script ownership) PLUS (b) Phase 4 document-review surfaced dep (path drift from `videos/trailer/public/` → `public/trailer/` per amendment TIER 1 #3; 8+ file refs in current Phase 5 plan need updating) PLUS (c) Phase 4 first-frame luminance dep (if gameplay clip frame 0 is bright, document the dependency on Phase 4's S05HeadFadeFromBlack overlay). Same 8-agent deepening shape + emil-design-eng + /brief as Phases 1-4._
_3. **Sequentially after Phase 5:** Phase 6 deepening, then Phase 7 deepening — same pattern, one per session._

_**Session state at end (2026-05-17 — this session):**_
- _Uncommitted changes: `docs/plans/origin-trailer/phase-4-remotion-composite.md` (~3900 lines = deepening pass + 11 document-review amendments applied), `docs/plans/origin-trailer/roadmap.md` (3 new ADRs #16/#17/#18 from prior deepening pass), `TODO.md` (this update reflecting amendments-applied state), plus pre-existing uncommitted deploy-migration files (board.html, player.html, public/_headers, src/client/howtoplay/acts/ActRemote.tsx, src/server/room.ts) deliberately quarantined per prior TODO landmine._
- _Scratch artifacts: `.context/compound-engineering/ce-plan/deepen/phase-4/` (gitignored; no longer load-bearing — amendments applied + checked off above)._
- _All 11 amendments verified against TODO §1 prescriptions before applying; sequential-thinking used for ordering + emil-design-eng framework loaded for design-judgment cluster (TIER 1 #4-6 + TIER 3 #11)._
