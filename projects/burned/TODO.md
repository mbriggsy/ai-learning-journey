...
| IncomingSteal banner (new, uncommitted pile from 2026-04-23) | On a real 3-of-a-kind named steal: target's phone rises "// INCOMING LIFT / {STEALER} / is lifting your / {CARD NAME}" banner DURING the 10s nope window (not just post-resolution). Countdown ticks, urgent-red flip at ≤2s, banner exits clean when window resolves. Verify bystanders see no banner and no card name anywhere. Injection test + post-resolution organic verified; **pre-resolution live organic screenshot never caught** due to Playwright tab-switch latency in this session. |
...
- **414/414 Vitest, typecheck clean** (re-verified 2026-04-23 EOS). Full Playwright not re-run after card-flip + End Game + Nope race + IncomingSteal work; run `pnpm exec playwright test` before push.
...
- **Pre-resolution IncomingSteal banner on target's phone (new 2026-04-23, uncommitted).** Closes the gap where a named-steal target only learned which card was on the line AFTER the steal resolved (too late to Intercept). `NopeWindowView` gained an optional `namedSteal: { stealerId, targetPlayerId, namedCardType? }` field — `namedCardType` is private per §13.8 house rule, projection allowlist exposes it only to stealer + target. `IncomingSteal.tsx` is the target-side banner that renders when `nopeWindow.namedSteal.targetPlayerId === myId && namedCardType != null` — countdown matches SmartActionBox, urgent flip at ≤2s, haptic on open, one-shot SR announcement per window generation. Sourced from `state.pendingNameCard` in projection (not `nopeWindow.pendingAction`) so the banner stays live across nope-chain windows. Projection is state-driven (not event-driven) so mid-window rejoin works. 5 new `src/server/projection.test.ts` tests lock the allowlist: board strips cardType, stealer+target get it, bystanders see only stealerId+targetPlayerId. **KNOWN UX GAP**: when the target already has queued `StealReport` incident overlays from earlier steals, the `StealReport` scrim visually obscures `IncomingSteal` even though z-indexes place the banner above. Consider gating `StealReport` behind `!nopeWindow?.namedSteal` or lowering its scrim opacity — not fixed in this pass.
