...
one (Otto) on the research budget: narrative-only-not-shipped per
Phase 1 line 1086 LOCKED narration + Phase 7 stat-verification gate
(ADR #26, locked 2026-05-17 during Phase 7 deepening; "in the
basement" framing corrected to "on the research budget" during
Phase 7 doc-review 2026-05-17 per design-lens D2 — Phase 1 line 49
was a pre-doc-review summary using the now-retracted "basement"
phrasing; locked narration at Phase 1 line 1086 + ActRoster.tsx:153-
158 is canonical). Captioned distribution copy must match: "six
operatives in the deck, one on the research budget."** + Dolores Grieves NPC,
...
| 19 | **Production encoding canonical lock — CRF 18 / `--x264-preset slow` / no `--tune`** (locked 2026-05-17 per Phase 6 deepening; cross-tool note added 2026-05-17 per Phase 7 doc-review) | Phase 6 deepening reconciled a three-way contradiction: Phase 0 ADR drafted CRF 16 for `render:final`; roadmap §3 row 6 said CRF 18; Phase 6 first-draft said CRF 17. **Lock at CRF 18** per industry visually-lossless threshold (FFmpeg H.264 wiki + slhck CRF guide). CRF 17 vs 18 is sub-perceptual on X's downstream re-encode; CRF 16 over-encodes for distribution. **Critical CLI flag fix**: `--preset slow` is **NOT a valid Remotion 4.0.438 CLI flag** — the correct flag is `--x264-preset slow`. The first-draft `--preset` would silently no-op and fall back to `medium`. **Cross-tool note (added 2026-05-17 per Phase 7 doc-review feasibility F1 + adversarial A1)**: the `--x264-preset slow` flag is the **Remotion CLI spelling only**. When calling raw FFmpeg argv directly (Phase 7 Unit 7.1 cutdown render, 9:16 render, any future ffmpeg-direct path), the libx264 encoder option is **`-preset slow`** (single dash, no `x264-` prefix). The two flags map to the same underlying encoder option through different tooling surfaces; do NOT use `-x264-preset` against raw `ffmpeg` (`ffmpeg ... -c:v libx264 -x264-preset slow` returns `Unrecognized option 'x264-preset'` — verified 2026-05-17 against installed FFmpeg 8.1). `--tune` omitted: mixed content (Imagen illustration + Playwright capture + Remotion chrome) doesn't fit `film` or `animation`; default psy-RD calibration is correct for heterogeneous sources. Full lock: libx264 High profile / CRF 18 / `--x264-preset slow` (Remotion CLI) or `-preset slow` (raw FFmpeg) / no `--tune` / yuv420p / 1920×1080 / 30fps / AAC 128k / **mono** (ADR #14) / +faststart / no maxrate cap (conditional addition only if Unit 6.1 Step 3 pre-test projects >280MB). Iteration loops use `pnpm render:iterate` (hardware-accelerated, ~3-5min); gold master uses `pnpm render` (software libx264 slow, ~15-25min on Apple Silicon / 15-30min Windows). Both gated by `pnpm verify:trailer-final` (Unit 6.0) before atomic-swap `.new` → mv. Phase 0 ADR `render:final` updated 2026-05-17 to CRF 18 + `--x264-preset slow` to align. |
...
| 26 | **Distribution stat-verification gate** (locked 2026-05-17 per Phase 7 deepening; "basement"→"research budget" framing corrected 2026-05-17 per Phase 7 doc-review) | Any numeric or roster claim in trailer captions, README copy, pinned-tweet copy, or thread copy MUST derive from a verifier script (`scripts/verify-caption-stats.ts`, Phase 7 Unit 7.0) that parses the codebase at write time. Hand-typed numbers banned. Verifier output `videos/trailer/distribution/verified-stats.json` is the canonical source; captions reference the JSON keys. Hard-asserts include `operatives_in_deck === 6` (matches Phase 1 **line 1086 LOCKED** trailer narration "Six in the deck. One on the research budget. Don't ask."). Otto-doesn't-ship invariant: verifier also asserts `cards.some(c => c.id === 'otto') === false`; if Otto ships, narration must re-evaluate. Narrative-locked counts live in `src/shared/narrative-stats.ts` (`OPERATIVES_OFF_BOOKS = 1`) imported by the verifier (per adversarial A7 Phase 7 doc-review — pre-doc-review verifier hardcoded the constant inline, defeating its own purpose). Fixes pre-deepening hallucinations: Phase 7 first-draft "7 operatives" (codebase `src/shared/card-defs.ts` has 5 operatives + Agent X wild = 6 entries); "Approximately 14,000 pages" follow-up tweet line (Phase 1 line 1584 explicitly flags as unverified — narration uses it as fictional in-character cadence; using it as a tweet stat passes it off as real → CUT). Multi-agent: adversarial (grep + count evidence) + repo-research (card-defs.ts line citations) + brief (institutional gap — no docs/insights/ entry on stat discipline at distribution boundary). Inherits `feedback-stats-single-source.md`. **Pattern propagates forward to next agentic-SDLC project distribution** (Phase 7 generates the pattern; no UMB v3 inheritance per brief's negative finding). |
...
| C7 | Phase 7 pre-deepening caption "7 operatives" (Variation A) | `src/shared/card-defs.ts` ships 5 operatives + Agent X (wild) = 6 entries; `public/assets/cards/` ships 6 operative `.webp` files (no `otto.webp`). Trailer narration (Phase 1 **line 1086 LOCKED** — Stat 4 at frame 1680) says "Seven on the roster. Six in the deck. One on the research budget. Don't ask." Roadmap §1 line 50 hallucinated a 7-operative roster matching narration ROLE-count without distinguishing deck-vs-off-books. ("Phase 1 line 49" was a pre-doc-review summary using the now-retracted "in the basement" framing — see Phase 1 lines 1100-1113 source-fix; ActRoster.tsx:153-158 is canonical.) | **Caption corrected to "six operatives in the deck, one on the research budget" per ADR #26 stat-verification gate (locked 2026-05-17 during Phase 7 deepening; "basement"→"research budget" framing corrected 2026-05-17 during Phase 7 doc-review per design-lens D2). Roadmap §1 amended inline 2026-05-17 to clarify Otto-on-research-budget is narrative-only-not-shipped. Future agentic-SDLC project distribution inherits the verifier-script pattern (`scripts/verify-caption-stats.ts`).** |
