...
> **Current test count:** see `TODO.md` §1 (canonical live count). Audit-snapshot baselines above are preserved as historical reference.
...
all 🔴 rows from the 2026-04-23 audit are now closed (2026-05-07 evening
sweep + C-21 + C-12 + C-07 + C-09 + C-10 + C-11 closures below). Visual rows need Briggsy-eye review
...
the 2026-04-23 audit are now closed (🟢 / 📋 / ⏸ / 🏷). Only ⏸ row
remaining: C-13 (asset regen budget — not engineering). All other ⏸
rows shipped 2026-05-07: C-15 by side-effect of `4985fa23`, C-16
subsumed by C-10, C-17 After-Action Report frame, C-18 Burn Notice
frame, C-19 Mission Briefing panel + Channel Ticker.
...
**C-11 closure 2026-05-07 (this commit):** left case banner's
`caseBannerStamp` removed — the dossier-folder cover's `coverStamp` is
now the singular CLASSIFIED-stamp surface on the board. Mechanism +
rationale documented in the C-11 row.

**C-10 closure 2026-05-07:** NopeCountdownBar moved out of
...
**C-07 closure 2026-05-07:** brass stand height bumped from
...
| **C-11** | DossierFeed side CASE FILE + inner CLASSIFIED stamp are two-of-a-kind competing for attention. **Shipped 2026-05-07 (this commit)** — left case banner's `caseBannerStamp` removed (also dropped its `stampDrop` keyframe and the matching `prefers-reduced-motion` entry). The dossier-folder cover's `coverStamp` is now the singular CLASSIFIED-stamp surface on the board. Why this direction: in real-world spy vocabulary rubber stamps belong on paper documents (the manila dossier folder), not on typeset wall posters / briefing boards (which is what the case banner reads as — Operation / BURNED / Case File / Briefed by M.). Plus the dossier cover's stamp is *stateful* — it disappears when events land (cover hinges rotateX -95°), giving it dramaturgy the static left-banner stamp couldn't match. Verified at 1920×1080: case banner reads cleanly as a typeset poster; dossier cover (when no events) carries the singular CLASSIFIED stamp. CLAUDE.md landmine note + TODO.md keyframes list updated to reflect `stampDrop`'s removal. | 🟢 |
...
| **C-18** | EliminatedView phone screen drops all Archer vocabulary. **Shipped 2026-05-07 (this commit) — Burn Notice frame.** Briggsy's pick from a 4-option menu; resolution adds: (1) header strip "// Burn Notice" + "[ Compromised ]" mirroring the GameOver After-Action Report bar so phone reads as the same agency-document layer across briefing → playing → burned; (2) mono-caps status caption "// Status: Cover compromised. HR notified." between the title and flavor line; (3) "Still alive" reframed as "// Active Operatives" with mono-caps drama-accent label and per-operative ALIVE status tags (mirrors GameOver's rank status pattern); (4) bottom prompt reframed "// Stand by for next briefing" in mono caps. Skull (💀) and Archer flavor lines (incl. "Penetrated by enemy assets. ...Phrasing.") preserved — they were the Archer-voice strength of the existing screen. Verified at 390×844: header fits clean single-line, skull + title + status + flavor + active-operatives panel + footer all visible without clipping. Phone now carries continuous agency typography across all four primary screens. | 🟢 |
