...
**Severity (triage):** P2
**Status:** ✅ RESOLVED
**Resolution:** Catalog-side recognition criteria tightening shipped 2026-05-09 in the commit closing this issue (Option A from the original triage). Three HIT-variant scenarios (`SCN-PAIR-OPERATIVES-HIT-01`, `SCN-TRIPLE-OPERATIVES-NAMED-HIT-01`, `SCN-TRIPLE-AGENTX-ONLY-NAMED-HIT-01`) now carry an explicit **"Do NOT self-report this scenario if:"** guard block following the positive recognition criteria. The guard makes three things unambiguous to the seat agent: (1) Counter window appearing means SOMEONE played Intercepted — outcome is undetermined until the resolution event; (2) absence of StealReport modal after nope-window close means the steal was cancelled and the HIT scenario MUST NOT be logged; (3) net hand-count delta is the unambiguous terminal — H-1 for pair-HIT, H-2 for triple-HIT — anything else means cancellation. The original triage's diagnosis (line 56 of this issue) said the existing criteria were inclusive-only — agents inferred fire from front-of-list signals (staged combo, nope window, counter window) without validating the terminal condition. The new exclusive guard explicitly enumerates the failure mode and points to the unambiguous terminals (StealReport modal observed AND hand-count net delta) as the only acceptable signals for a HIT-variant fire. No engine, projection, or oracle change — pure catalog edit. Future playtests with seats running against the updated catalog should not surface false-positive HIT fires on intercepted outcomes; if they do, the next escalation is Option C (oracle-side self-report validation in the detector pipeline), tracked here as future hardening if the catalog guard alone isn't sufficient.
**Original disposition (pre-fix):** Bundled into Cluster I (harness oracle false-positives) for 2026-05-09 review. The existing recognition criteria already require both "watched the nope window expire WITHOUT an Intercept" and "received a StealReport naming the specific card that moved" — seat-3's session record shows neither condition was met (the steal was intercepted, no StealReport surfaced), so the criteria did NOT permit the self-report. This is a seat-agent FIDELITY issue (agent reported despite criteria-mismatch), not a catalog or oracle gap that can be fixed by tightening the criteria text. The structurally-correct fix is in the coverage-reporter or seat-agent prompt enforcement — separate scope from the 015 + 002 oracle-snapshot fix that landed in the same commit. Coverage-reporter already logs `seat-without-detector` divergences for these cases (`scripts/playtest/lib/coverage-reporter.ts:188-204`), so the false-positive does NOT inflate the production fire count — it surfaces as a coverage divergence note. Tracking as known limitation pending a dedicated seat-agent fidelity / coverage-filter pass.
