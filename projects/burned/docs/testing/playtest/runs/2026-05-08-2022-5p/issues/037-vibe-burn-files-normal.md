...
**Severity (triage):** P2
**Status:** ✅ RESOLVED
**Resolution:** Asymmetric ember-breath shipped 2026-05-09 in the commit closing this issue. `DramaOverlay.tsx` gains an `appendEmberFlicker` helper that runs in parallel with the hold window, anchored to a new `HOLD_START_LABEL` that `appendHoldAndExit` plants. Two layers, both with one asymmetric arc across the entire `holdMs` (no metronomic pulses): (1) target text `--ember-pulse` 1.0 → 1.18 → 1.0 with 5/8 inhale `sine.out` + 3/8 exhale `sine.in` — the CSS var modulates `.burnedfiles .text` text-shadow blur radius + color-mix alpha so the glow halo flares then settles without touching the type's own filter; (2) overlay `filter: brightness()` 1.0 → 1.06 → 1.0 with 17/24 inhale + 7/24 exhale — slightly slower flare so the layers don't lock cadence. Eye-in-loop verdict: "way better" — strobe gone, reads as breath. Design driven by Emil-design-eng review (skill invoked twice): V1 metronomic 280ms-half-period yoyos read as ~4 strobes inside the 1200ms hold; V2 single asymmetric breath replaces strobe with one perceived motion arc. A V3 sub-perceptual `scale` crackle layer (1.000 ↔ 1.004 at 60ms half-period) was prototyped as fire-alive noise floor and A/B-tested against the breath alone — verdict: indistinguishable, dropped per Emil's "every layer earns its keep" rule. Crackle pattern preserved in git history (search `CRACKLE_HALF_SEC`) for resurrection if a future playtest reports breath-alone reads inert. Three layers/axes were considered (text glow, overlay brightness, scale jitter); two ship. Pinned by `tests/e2e/drama-beat-timing.spec.ts` runtime motion gate (text/overlay never animate opacity, so `peakSustainedMs ≥ 60% of holdMs` survives) — chromium gate green 3/3 on final state. Pinned follow-ups (Emil framework): next-day fresh-eyes review and real-device phone-hardware pass — see `TODO.md` §4 carryover.
**Original disposition (pre-fix):** Narration baseline restored 2026-05-09 by #036's PlayerAlert `deck-shuffled` toast. The remaining ask (GSAP ember-pulse during the DramaOverlay holdMs to lift the beat from "status subtitle" to "fire is active") is motion design that needs eye-in-loop verification — Briggsy memory `feedback-eye-in-loop-beats-calibration-for-motion.md` warns that calibration agents poll DOM state but can't FEEL motion. Doing this autonomously would risk shipping unverified vibe work. Tracking as motion-calibration scope alongside the broader DramaOverlay polish queue. Pinned by `tests/e2e/drama-beat-timing.spec.ts` — any future motion change must keep the runtime gate green.
