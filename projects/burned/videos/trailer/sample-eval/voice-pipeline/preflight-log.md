# Phase 2 preflight log

Per-invocation preflight result. Voice IDs truncated to 8-char prefix
(account-scoped credentials never land in tracked artifacts).

## Preflight 2026-05-19 00:40
- node: 24.13.1
- ffmpeg: 8.1 / ffprobe: 8.1
- engine: elevenlabs-v3 (raw: "ElevenLabs v3 (Voice Library preset; Sterling-CODED voice")
- model: eleven_v3
- cleared path: A (raw: "A (Voice Library preset)")
- R5 outcome: kept-via-A
- dash voice: CwhRBWXz...
- cold-open speaker: janet (voice: m8AHWg36...)
- cadence adapter: sample-eval/r4-dash/cadence-spec-elevenlabs.json
## Preflight 2026-05-19 00:41
- node: 24.13.1
- ffmpeg: 8.1 / ffprobe: 8.1
- engine: elevenlabs-v3 (raw: "ElevenLabs v3 (Voice Library preset; Sterling-CODED voice")
- model: eleven_v3
- cleared path: A (raw: "A (Voice Library preset)")
- R5 outcome: kept-via-A
- dash voice: CwhRBWXz...
- cold-open speaker: janet (voice: m8AHWg36...)
- cadence adapter: sample-eval/r4-dash/cadence-spec-elevenlabs.json
## Preflight 2026-05-19 00:41
- node: 24.13.1
- ffmpeg: 8.1 / ffprobe: 8.1
- engine: elevenlabs-v3 (raw: "ElevenLabs v3 (Voice Library preset; Sterling-CODED voice")
- model: eleven_v3
- cleared path: A (raw: "A (Voice Library preset)")
- R5 outcome: kept-via-A
- dash voice: CwhRBWXz...
- cold-open speaker: janet (voice: m8AHWg36...)
- cadence adapter: sample-eval/r4-dash/cadence-spec-elevenlabs.json
