...
**Status:** Deepening in progress. **Phases 0–7 AND the preflight plan (−1) are deepened + doc-reviewed.** Phases 8–9 remain to deepen. **No code until ALL phase plans are deepened and contradictions resolved** (house rule).
...
- [ ] **Phase 8 — deploy** (`phase-8-deploy.md`) — **NEXT.** Must resolve the open "where does `pnpm refresh` run" question (a clean CI runner has NO session JSONLs → tokens would be null; leading option = refresh locally + commit, Action only deploys — see phase-2 Open Decision #2). Also owns the README-line-71/§8.3/About-§5 cadence reconciliation (landmine below).
