...
> **`stats.json` is committed, not gitignored.** The README locks "static JSON committed at build time"; Vercel serves the committed file. Phase 1's `.gitignore` does NOT ignore `public/` — confirm `public/data/stats.json` + `public/assets/<name>/` are staged. (The Phase 8 GitHub Action commits the regenerated file on each push.)
...
2. **Phase 8 runs on a Linux GH-Action runner that has NO session data.** `refresh-stats` calls `buildMultiProjectReport({})`, which reads `~/.claude-credit-projects.yaml` + `~/.claude/projects/*.jsonl` from the **runner's** home — neither exists on a clean CI runner → published `tokens` would be **null**, and the hero's token number would be empty. This may invalidate the locked "GitHub Action refreshes on every push" cadence: the session data only lives on **your** machine. **Options: (a)** run `pnpm refresh` locally (e.g. in `squeaky-clean`) and commit `stats.json` — the Action only deploys, never regenerates tokens; **(b)** the Action regenerates only the git/asset data and accepts null tokens in CI; **(c)** ship session data to the runner (rejected — that's exactly the PII we strip). **Lean: (a).** This is a real Phase 8 architecture decision — not fixable in Phase 2, flagged here so Phase 8's deepening resolves it.
...
4. **Drop `commitsByAuthor` from the published JSON too?** It ships raw `%aN` git author names and is **redundant** with `linesByAuthor` (which the site actually reads for AUTHORED BY). Dropping it (alongside `warnings`) shrinks the public name surface. Phase 0 treats author names as public-safe ("already public on GitHub commits") — true for public repos. **Lean: drop it** unless a surface needs it; coordinate with Phase 0's `ALLOWED_KEY_PATHS`. Low stakes.
...
### `phase-8-deploy.md`
- **★ Where `pnpm refresh` runs is an open architecture decision** (Open decisions #2): a clean Linux GH-Action runner has no `~/.claude/projects/` session data, so CI-side refresh would publish null tokens. Phase 8 must resolve this — leading option is to regenerate `stats.json` **locally** (committed via `squeaky-clean` or a manual `pnpm refresh`) and have the Action only deploy. This reshapes the "Action refreshes on every push" cadence the README locked.
