...
| 3 | Honest sub-line | `combined.totalTokensFresh` + `combined.tokenWindowDays` | `--text-meta` or `--text-body`, `.tabular` on numbers | `--text-muted` | The retention-honesty receipt. Suppress retention clause if `tokenWindowDays === null`; "< 1 day" if `0`. |
