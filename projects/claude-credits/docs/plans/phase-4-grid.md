...
9. **Reveal via `ScrollTrigger.batch`, not per-element triggers** (verified against GSAP AI-skills docs via Context7, 2026-05-24). Everything below lives INSIDE one `useGSAP(() => {…}, { scope: gridRef })` block so the dev StrictMode double-invoke reverts the `gsap.set` and kills the batch cleanly (mirror Phase 3's HeroCounter):
   ...
   - **`refreshInit` y-reset (load-bearing):** `ScrollTrigger.addEventListener('refreshInit', () => gsap.set('[data-tile]', { y: 0 }))`. The hidden `y: 40` offset displaces each tile's measured top by 40px during `refresh()`, so `start: 'top 85%'` fires at the wrong scroll position (and compounds the stale-position bug Decision 10 fixes). `batch` cannot take `invalidateOnRefresh`, so `refreshInit` is the only mechanism to revert the offset during measurement. `autoAlpha` (opacity/visibility) doesn't affect layout, so only `y` needs resetting. (This is the GSAP batch advanced-example pattern.)
   - **StrictMode note:** because the `gsap.set` + `batch` + listener all live in the `useGSAP` scope, the dev double-invoke add-revert-add cycle is clean — no leaked trigger, no double-hide. Verify the top row is visible after mount in **dev** (StrictMode on) AND prod preview.
...
- `ScrollTrigger.addEventListener('refreshInit', () => gsap.set('[data-tile]', { y: 0 }))` — y-reset during measurement (Decision 9).
- `ScrollTrigger.batch('[data-tile]', { start: 'top 85%', once: true, onEnter: els => gsap.to(els, { autoAlpha: 1, y: 0, duration: duration.reveal, ease: 'weighted-arrive', stagger: stagger.tiles, overwrite: true }) })`.
...
| **StrictMode double-invoke re-hides above-the-fold tiles in dev** | `gsap.set` + `batch` + `refreshInit` listener all inside `useGSAP({scope})` → dev double-invoke reverts cleanly. Dev (StrictMode) verify is separate from the prod P0 gate. |
...
- **Authorship "AUTHORED BY" block** reads `linesByAuthor` — gated on the autonomy reconciliation pass + git-attribution landmine (below). Phase 5 must not ship a claim before that pass resolves it.

### Autonomy reconciliation pass (separate deepening — flagged here, owned next)
This Phase 4 conversation surfaced a site-spine truth bigger than the grid: **the human authored 0 lines (only `.env` API keys); this monorepo is fully-autonomous-SDLC experiments.** Two pieces to resolve across `ideation.md` §1/§2, `phase-0-data-gaps.md`, `phase-3-hero.md`, `phase-5-detail.md`, `phase-6-about.md`:
1. **Correctness — the git-attribution inversion:** `mbriggsy` is the git commit author; Claude rides as `Co-Authored-By`. Naive `linesByAuthor` credits the human as primary author — the inverse of the truth. Phase 0's `linesByAuthor` must decide how to attribute "Claude wrote it (Co-Authored-By)" vs "Briggsy committed it," or any authorship surface ships a claim its own data contradicts. (Phase 4's sort + fallback hook sidestep this — they use `grandTotals.authoredLines`, a file-classification metric, NOT `linesByAuthor`.)
2. **Framing:** "fully autonomous SDLC — the human wrote none of it" is a headline-grade claim. The hero (on this same landing page, above the grid) is its natural home; the About page is the deep version. Decide how loud and where.
(Not Phase 4 work — Phase 4 puts NO authorship viz on the tile. Resolved in the autonomy pass before any code executes.)
