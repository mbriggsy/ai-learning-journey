...
Phase 5 tells **one project's story** as a single-column editorial scroll — magazine opener, then the work, then the rhythm of how it got built. It is the first **route-param-driven** consumer of `useStats()` (the hero read `combined.*`, the grid iterated `projects[]`/`meta[]`; this page resolves ONE report by name and 404s honestly when there's no match). The single visual flourish is the **AssetDonut DrawSVG reveal** — everything else is type, negative space, and quiet block reveals.
...
12. **CONTENT FLOOR — the page is verified against the DATA-SPARSE project, not just BURNED** (doc-review, adversarial + design + the through-line finding). The plan must not be designed only against the data-rich case. The worst real case is the **`claude-credit` meta project on a clean deploy box**: `tokens: null` (no JSONL on the runner), `assetBytesByKind` all-zero (no pipeline media), a thin `tiers[]` (code + tests + a README), and — until the editorial worksheet runs — `editorial: null`. With everything null, Movements 2 (tokens/gold), 3 (description), 4 (donut), 6 (gallery), 7 (invitation) all omit → a bare hero + sparkline + a 1–3-item inventory. That is the flagship project the AI-peer audience clicks FIRST (they came to geek on the tool). Two guardrails:
   - **EDITORIAL IS A HARD DEPENDENCY for any project with a detail page.** Every project in `projects[]`/`meta[]` MUST carry an `editorial` block with at least `oneLiner` + `description` (sourced in preflight −1.5 worksheet + each `claude-credit.config.yaml`). That guarantees the **opener one-liner + the description prose breath always render**, so the page always tells a STORY even when token/media data is sparse. This makes the editorial worksheet a Phase-5 blocker, not optional polish. (`claude-credit` and `claude-credits` — the two meta projects — especially need rich editorial, since they're the most data-sparse AND the most-clicked.)
...
- `useStats()` returns a NON-NULL `MultiProjectReport` (Phase 2): `{ projects, meta, archiveCollective, combined, scannedAt }`.
- **Project lookup:** find the `ProjectReport` in `[...projects, ...meta]` where `projectName === name`. Archive entries are NOT here (collective only) → a shelved name finds nothing → not-found (Decision 8).
- Per `ProjectReport` the page consumes:
  - `projectName` (hero title), `kind` (unused for layout — both active and meta render identically).
...
### Concrete responsive + render specs (doc-review — paint-by-numbers values an implementer must not guess)
...
- A tiny `findProject(report: MultiProjectReport, name: string): ProjectReport | null` — searches `[...projects, ...meta]` by `projectName`, returns `null` if absent (drives the not-found fork). Pure, trivially tested. (Lives here or in a `lookup.ts`; co-locating with composition is fine.)
...
- `findProject`: Happy — finds in `projects`; Happy — finds in `meta`; Edge — a name only in `archiveCollective.projectNames` → `null`; Edge — unknown name → `null`; Edge — empty arrays → `null`.
...
| **Data-sparse project renders a near-blank "detail"** | The data-sparse case (`claude-credit` meta on clean deploy: tokens null + no media + thin tiers) omits 5/7 movements. Guard: Decision 12 — editorial (`oneLiner`+`description`) is a HARD dependency for every detail-page project, so the opener + prose always carry the story; the inventory has a ≤3-item layout floor. Verify the combined-null case + the meta project specifically. |
...
- **First route-param consumer of `useStats()`.** Hero read `combined.*`; grid iterated `projects[]`/`meta[]`; the detail page resolves ONE report by `:name` and is the first surface that can legitimately find NOTHING (not-found fork). It honors field-level null discipline per-section.
...
- **Final donut palette + sparkline curve tuning + negative-space pass** → Phase 9 (Phase 5 ships a correct, on-spec first build with placeholder-safe semantic colors).
...
1. ✅ `pnpm test` green — `composition.test.ts` (curated kinds + unit mapping + zero-omit + missing-node + no-mutate; `findProject` projects/meta/archive-name/unknown/empty) and `format.test.ts` `formatShortDate`; Phase 2/3/4 tests still green.
...
4a. ✅ **Data-sparse / combined-null (Decision 12):** a project null on tokens+media+gallery at once — AND the real `claude-credit` meta project — still reads as a composed story (opener + description prose + sparkline + ≤3-item inventory as a centered row, no empty grid columns); never a broken stub. Editorial (`oneLiner`+`description`) is present for every detail-page project.
