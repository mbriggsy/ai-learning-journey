...
3. **The install block (§2) reads the resolved CTA state from `editorial.md`'s `## CTA state` block — never hardcoded, with a safe fallback** (the same single source Phase 7's primary CTA reads). Preflight −1.1 (is `claude-credit` publishable?) is **unresolved** at this deepening (`npm view claude-credit version` not run), and `editorial.md` does not exist yet (it's authored in preflight −1.5, which owns creating the `## CTA state` block and even handles out-of-order creation). The block must render whichever state −1.1 lands:
   ...
   - **STATE UNRESOLVED (degenerate, must handle):** if the `## CTA state` block is **absent or still carries placeholder tokens** (e.g. the literal `<one line>`), render **STATE B copy as the safe default** AND fail loudly in dev (console error / build warning) — **never an empty mono box and never a literal placeholder string**. Plans are menus not orders; nothing structurally forces preflight-before-Phase-6, so this path is real.
   - **Parity with Phase 7 = same install string, not same block.** Phase 7 is **undeepened** and currently sketches STATE A plus a `claude-credit --all --json > stats.json` first-run line; About §2 is **install-only** and deliberately does NOT mirror that first-run line. "One resolution, two surfaces" means both read the same `## CTA state` install string — it does not mean identical layout. **Do NOT hardcode STATE A**, and don't assume a shared component exists yet (the shared surface is the `editorial.md` block, read at build time). The shipped `tools/claude-credit/README.md` carries an `npm link` install line that *implies* published — preflight −1.1 owns hedging it; §2 reflects the resolution, never asserts published.
...
| 2 | What is claude-credit + install | prose + resolved `editorial.md ## CTA state` | heading + prose; STATE A/C → install line in `--font-mono` selectable `<code>` block (no copy button v1); STATE B → prose only, no block | STATE B → no mono block; **STATE UNRESOLVED → STATE B default + fail loud** (Decision 3); never empty box / literal placeholder |
...
| **Install snippet** (§2) ← preflight −1.1 + `editorial.md ## CTA state` | OPEN (−1.1 not run) | §2 reads the resolved CTA-state block; STATE-UNRESOLVED → STATE B default + fail-loud (Decision 3); never hardcode STATE A. |
...
| **Phase 7 undeepened** — "parity" claim has no shared component yet | Phase 7 = 1KB sketch | Parity = same install string read from `editorial.md`, not same block; About §2 install-only (no first-run line) (Decision 3). |
...
**Test scenarios:**
- *Happy path (install block):* CTA state A → `npm i -g claude-credit`; C → `npm i -g @mbriggsy/claude-credit`; B → prose + source link, no install line. **If Phase 7 already unit-tests this state→snippet mapping, inherit it — do NOT duplicate;** verify About's block by eye-on-browser. If no shared test exists, add ONE (in a shared lib or Phase 7), not a parallel suite here.
...
- **API surface parity:** §2's install string and Phase 7's CTA primary read the same `editorial.md ## CTA state` block — parity means same install string, NOT same layout (About is install-only; Phase 7 carries the extra first-run line). Phase 7 is undeepened; the shared surface is the editorial block, not a component.
...
- **Install-snippet source + states:** `docs/plans/phase-7-cta.md` (undeepened), `docs/plans/phase-preflight.md` (−1.1 states A/B/C, −1.5 editorial worksheet)
