...
- **Scope-tag at mint (his ask, 2026-07-11):** every rule carries `[briggsy-general]` (how he
  reads ANYTHING — portable to a future global caddie-builder extract) or `[product]` (this
  product's scared-couple finance tone — never transfers). Tag AS-WRITTEN: a product-worded
  rule tags `product` even when its principle generalizes — the extractor rewords, it never
  silently re-scopes. Exemplars/anti-patterns are incident-bound and never transfer wholesale.
...
1. **Unquoted quantity.** `[briggsy-general]` Flag any sentence referencing a quantity (threshold, income,
...
2. **Endpoint-less delta.** `[briggsy-general]` Flag a distance/gap shown without BOTH endpoints riding alongside.
   "~$174,300 of room" alone fails; "~$43,700 income, line at ~$218,000, so ~$174,300 of room"
   passes.
3. **Mechanism-frame vocabulary.** `[product]` Flag copy in the mechanism's frame instead of the
...
4. **Unattributed derived figure.** `[briggsy-general]` Flag a modeled number presented as if the user entered it.
   A computed figure names its source ("the plan expects about ~$X"), never a bare "your income".
5. **Garden-path sentence.** `[briggsy-general]` Flag em-dash appositions and multi-fact sentences on load-bearing
   figures. Dollar sentences are subject–verb–number, one fact each.
6. **Era-naked twins.** `[briggsy-general]` Flag two same-named quantities with different values that don't each
...
7. **Certainty language.** `[product]` Flag "guarantees", "no matter what", "unbreakable", "you're covered"
   in any verdict/projection copy. Probabilistic framing only (R12).
37. **Disclosures name the mechanism, never the reader's memory.** `[briggsy-general]` Flag any disclosure whose
...
38. **General terms require a details home.** `[briggsy-general]` A surface may speak a quantity in general terms
...
8. **Calm-but-wrong outranks everything.** `[product]` An optimistic/rosier reading is WORSE than an
...
9. **Relief-with-honesty is ONE stacked answer.** `[product]` On a two-tier surface the harder/safer
...
10. **No alarm on the relief beat.** `[product]` A security ritual or warning auto-firing onto the
    magic-moment verdict is alarm-after-calm whiplash — must be user-initiated and calm.
11. **Bad news reads as honest recalibration.** `[product]` A worsening answer recalibrates calmly — never
...
12. **No side quests.** `[briggsy-general]` A user-facing sub-task that matters must be grammatically in-flow and
...
13. **Unpriced risk wears a disclosure + a capped grade.** `[product]` Any cost the model can't yet price
...
17. **Proven vs believed.** `[briggsy-general]` Reject absolute reliability claims and marketing voice about the
...
18. **Never color alone.** `[briggsy-general]` Every meaning (safe/danger, cliff, over/under, verdict) carries a
...
20. **Broken layout first.** `[briggsy-general]` Report clipping, overflow, off-frame elements, unreadable labels,
...
21. **Dead whitespace = failure.** `[briggsy-general]` Every pixel earns its keep at the real viewport. Neutral
    emptiness is a defect, not "breathing room".
22. **Live regions reserve their box.** `[briggsy-general]` A live-updating region above interactive controls
    without a fixed min-height shifts tap targets mid-gesture (insight 035). Reward/relief copy
    stays OUT of role=status.
23. **Overflow spends whitespace, never disclosures.** `[product]` When content overflows the one-frame
...
24. **Content cuts are product decisions.** `[briggsy-general]` Flag any deletion/fold/"summarize" done to make a
...
36. **Every chart speaks one dialect.** `[briggsy-general]` Axis grammar, tick style, hover treatment, and clock
...
25. **CSP-legal motion only.** `[product]` Verdict/transition animation is CSS class transform/opacity —
...
26. **Feel-words mean a timing bug.** `[briggsy-general]` "Camera flash", "feels rushed", "blink" — instrument the
...
27. **Water-beads self-check.** `[briggsy-general]` Would the most critical user say WOW — and does the "wow,
...
28. **Never cut richness for a non-correctness reason.** `[briggsy-general]` When a simplicity instinct wants to
    cut visual richness, flag the CUT. Correctness concerns (bugs, races, perf) remain valid.
29. **Mobile shines, never survives.** `[briggsy-general]` Every surface gets a phone read as a peer to the laptop
    read. "This is a desktop showcase" is the bug.
30. **Feel outranks systems.** `[briggsy-general]` Comprehensive tests with weak presentation = fail. "Catch your
    breath" good is the bar; green counts nobody can see earn nothing.

### Process (how a verdict must be produced and presented)

31. **Never present unverified.** `[briggsy-general]` No "ready to go" without eyes on the real rendered output at
    the real viewport. If it wasn't visually verified, say so.
32. **The cold read is a gate, not a courtesy.** `[briggsy-general]` Tests cannot see comprehension holes by
...
33. **Measure before fixing.** `[briggsy-general]` Before any layout/CSS fix: computed values in the FAILING
    environment, ONE fix, verify, stop. One "stop" from him means it already went too far.
34. **The taste oracle is N=1.** `[briggsy-general]` Flag any plan reaching for human panels, outside reviewers, or
...
35. **Lead with one educated recommendation.** `[briggsy-general]` Present forks as ONE rec + reasoning, anchored
...
39. **Civilians read omission lists as bills.** `[product]` Every "isn't counted" item invites "why not?
