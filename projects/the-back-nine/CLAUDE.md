...
| `pnpm verify:fit` | Real-Chromium **vertical-fit** gate (`e2e/vertical-fit.spec.ts` on `pnpm dev` — the one-frame fit law + the date-route order contract; own harness `playwright.fit.config.ts`) **+ the chart-text gate** (`e2e/chart-text.spec.ts`, same harness: every band / ladder / TwoFutures word ≥ `--text-xs` at the leaf, inside its named card, never overprinting, the scrub readout inside the plot at every lattice column, on PHONE/320/FLOOR/REAL; RecommendationViz is unit-pinned only until its own serialized solve arm lands — [`docs/architecture.md §12`](docs/architecture.md)) |
| `pnpm verify:doc-stats` | Doc-number **drift** gate: the test count (README + roadmap must match the live `vitest` suite), the register's open count (`docs/backlog.md`'s header must match its own body, and may be typed nowhere else), the insights index (every insight file indexed, every indexed file exists), and every line-numbered code citation in the live docs (the file exists, the line is in range and not blank; archived `<details>` blocks and the dated logs exempt — the semantic half, "is the cited THING still there", is the anchor fleet's job at unit boundaries) |
