...
| `pnpm verify:doc-stats` | Doc-number **drift** gate: the test count (README + roadmap must match the live `vitest` suite), the register's open count (`docs/backlog.md`'s header must match its own body, and may be typed nowhere else), and the insights index (every insight file indexed, every indexed file exists) |
