...
5. **Backlog — *everything still open*** (the full tiered register — the open COUNT lives only in that file's own header, never re-typed here (it rotted twice); consolidated 2026-08-02 from a source audit + an archive salvage and re-anchored the same day against the shipped code; each traced to its raw obligations): [`docs/backlog.md`](docs/backlog.md).
...
| `pnpm dev:phone` | Vite dev (mode `phone`, port 5174) over self-signed HTTPS — use this for a LAN phone (`https://<LAN-IP>:5174`, accept the certificate warning). Plain `pnpm dev` is LAN-exposed over HTTP (`server.host: true`), and a phone on it has no secure context: WebCrypto is absent and the `?vault=` plant/unlock renders footer-only. Never the default (the fit/caddie/CSP harnesses probe `http://127.0.0.1`) |
| `pnpm build` | Typecheck + production build |
| `pnpm preview` | `vite preview` of `dist/` — does NOT apply `vercel.json`'s CSP headers (use `verify:csp` for enforcement) |
...
| `pnpm verify:state-tax` | State-income-tax **re-verify** gate for the priced roster {NC, PA, FL} (mirrors `verify:aca`; all three sit on the same **annual** drift cadence but each on its **own** `nextDue` — the gate judges every record separately, so it reds in two waves and the roster's real deadline is the earliest of the three; NC's ~Aug-2026 rate-certification checkpoint was retired 2026-08-02 by S.L. 2026-41 § 44.1(a)) |
| `pnpm verify:csp` | Real-Chromium **CSP enforcement** gate (`e2e/csp.spec.ts` serves `dist/` through `vercel.json`'s headers — a blocked inline script + a blocked cross-origin fetch, each with a no-CSP control arm) — plus, since 2026-09-08, a second `webkit` Playwright project that greps `@cross-browser` and re-runs the two `e2e/vault.spec.ts` arms under WebKit (`scripts/__tests__/playwright-projects.test.ts` pins the project to the tag so it can never collect zero tests and stay green). This harness collects `e2e/**` by a `testIgnore` DENYLIST, so an unlisted spec lands here by default — since 2026-09-11 `scripts/__tests__/playwright-harness-partition.test.ts` runs every config's real collector and proves each `e2e/**` spec is claimed by exactly ONE harness and nothing under `e2e/held/` by a CI gate |
| `pnpm verify:fit` | Real-Chromium **vertical-fit** + **chart-text** + **phone intake-fold** gates — the one-frame fit law, every band / ladder / TwoFutures word legible at the leaf, and the intake step's scroll-reset / editor-heading / nav-yield laws (`e2e/vertical-fit.spec.ts` · `e2e/chart-text.spec.ts` · `e2e/intake-fold.spec.ts`, own harness `playwright.fit.config.ts`, against `pnpm dev`). What each arm asserts lives once in the roadmap's [Validation-gates](docs/roadmap.md#validation-gates) table's "Vertical fit + chart text" row; the chart-text CONTRACT is [`docs/architecture.md §12`](docs/architecture.md) |
| `pnpm verify:fit:rv` | Real-Chromium **RecommendationViz chart-text** gate on its own **serialized** harness (`e2e/chart-text-rv.spec.ts` · `playwright.fit-rv.config.ts`, `workers: 1`, port 4192, its own CI job): the fourth chart exists only after a full-precision solve (~6–8 min per arm, PHONE/FLOOR/REAL, primary + runner-up), so beside `verify:fit`'s arms it would starve them of cores. Same roadmap row as `verify:fit`; the shared audit core is `e2e/chartTextAudit.ts` (a spec never imports another spec) |
| `pnpm verify:doc-stats` | Doc-number **drift** gate — five arms: the test count, the register's open count, the insights index, every line-numbered code citation in the live docs, and every numbered insight's four required sections. What each arm checks lives once in the roadmap's [Validation-gates](docs/roadmap.md#validation-gates) table's "Doc-number drift" row |
| `pnpm caddie:walk` | The Caddie capture harness (`playwright.caddie.config.ts` → `e2e/caddie-walk.spec.ts`, port 4195) — driven by the `caddie` skill, not a CI gate |

## Layers & import boundaries (ESLint-enforced)

`engine · crypto · store · intake · budget · viz · ui · shared` (path aliases `@engine/*` … `@shared/*`).

- **`src/engine/` is PURE.** A deterministic function of `(params, seed)`. It imports only `@shared`; it must not import ui/store/intake/budget/viz/crypto. It reads **no clock, entropy, or environment** — `Math.random`, `crypto.getRandomValues`, `Date`, `performance`, and `process` are all lint-banned inside `src/engine/**` (the seed is **injected** by the caller). Only files inside a `__tests__/` directory are exempt; a `*.test.ts` placed directly beside an engine module is linted as engine code.
...
- **Reduce-to-spine invariant.** Every overlay (tax, healthcare, earned-income bridge, accumulation) reduces **byte-identically (same seed)** to the Trinity/Bengen-validated decumulation when OFF — accumulation's OFF is **PRESENCE-keyed** (the construct ABSENT from params; a zero-valued-but-constructed run is deliberately NOT byte-identical). The golden cases are never perturbed.
...
- **ONE canonical, year-keyed table** (burned/057,061,063). Plan, overlays, and tests all **read** it — a dated figure is never re-typed elsewhere (a shape test greps for inlined values).
...
- **Toolchain:** mirrors `projects/burned` — `pnpm@10.30.3`, TS `~5.9.3` (tilde — a TS minor can't silently change type-checking under a correctness-critical engine), Vite 8 (`rolldownOptions`, `resolve.tsconfigPaths`), Vitest 4 (`globals:false`), flat ESLint 10, **no Prettier**. Co-locate `*.test.ts` in the module's sibling `__tests__/` directory (the only lint-exempt test location); property tests `*.pbt.test.ts` via `fast-check`.
