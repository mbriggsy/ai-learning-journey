...
0. **Doc restructure — GREEN-LIT, mid-flight. M1 + M2 DONE.** Full ground-up rewrite. Foundation (CP1 + CP1.5), **M1 (the migration ledger)**, and **M2 (the four hubs)** are committed. M2 re-authored `product`/`roadmap`/`architecture`/`glossary` present-tense from the ledger: SS sub-engine folded into architecture §7.7 (+ longevity §7.6); accumulation kept as first-class engine content; R40 de-special-cased; all four changelog sections killed; glossary SS terms repointed → architecture §7.7; zero dead links; 942 tests green. **Resume at M3** — create the three NEW `decisions/` records (`ss-computation`, `other-income-r40`, `portfolio-holdings`) from the ledger's `decisions/` home-sections + reframe the existing `accumulation-fuck-off-date` record present-tense; then **linkify** the three inline-code decisions references the M2 hubs left (read [`RESTRUCTURE-PLAN.md`](RESTRUCTURE-PLAN.md) §"RESUME HERE" for the exact spots + the two carry-forward notes). Do NOT delete `features/` or strip history until M6's zero-loss gate. Delete `RESTRUCTURE-PLAN.md` + `.recovery/` when it lands.
