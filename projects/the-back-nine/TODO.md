...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   *"One modeling choice: these prices step up with your ages, not with the way plan prices themselves climb
   ...
   (gated on `statePriced` alone — `copy.ts:1105-1107`). Unfiled: `shadowRateHeadroom` (`copy.ts:2603`)
...
3. **A household outside {NC, PA, FL} gets a confident winner computed with zero state income tax.**
   ...
   **His scope call.** ⚑ Two corrections from the 2026-08-02 audit: (a) the honest-withhold precedent it
   ...
   the compile tie.** `_V3FieldsCover` (`model.ts:2303-2305`) covers only `keyof ScenarioV3`;
...
12. **The couple's own data.** ✅ **The warn half SHIPPED 2026-09-03** (the review's residuals sit in the
    ...
    copy" and the two nearest register entries are owned **council** (`backlog.md:790`) and **pilot** (`:836`) — neither is a copy fix; the "three arms with
    ...
    `copy.ts:2507-2509` under the NUMBER AGREEMENT post-mortem at `copy.ts:2493`, covered by `copyGuard.test.ts:637`):
    ...
    PRESCRIBED: `model.ts:1393-1395` mints synthetic entered accounts from the old aggregates "so the ladder
...
16. **The gates that don't bite (14 filed bullets — 13 distinct, the last two the same deferral filed
    ...
    clause was swept in `staleness.ts:550-553` but still lived verbatim at `model.ts:2260` (swept), and
    Plan 4's "the record carries `seedA`/`seedB`" (`plans/4-recommendation.md:282`) had outlived its own
    "kill BEFORE S5 mints" deadline — the shipped `SavedRecommendationV3` (`model.ts:1885-1907`) has no seed
    ...
    all green. (`copy.ts:2889` and `caseStateCompanions.ts` were already correct — swept 2026-08-02.)
...
---
    ...
    `copy.ts:2936`; a new `recViz` prefix would red the three hedge-free arm labels — each measured to red
    ...
    those two arms. Two half-swept false comments closed 2026-09-04: `model.ts:2260` and
