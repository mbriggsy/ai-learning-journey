...
`copy.ts:2683`-at-filing → `copy.ts:2983` (`recDeltaVizAria`) + `recommendationView.ts:681-687` → `:697-703` (u17 saved-rec build
spec) and `copy.ts:2682`-at-filing → `copy.ts:2983` (this file's item-10 entry; the "-at-filing" numbers are the RECORDED stale
...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   + `:1731-1738` → `healthOverlay.ts:270`. (b) `copy.ts:1159` is a Medicare eyebrow; the strings that claim
   ...
   both lists (`copy.ts:1112-1125`); what is genuinely unmodelled about it is the COST TREND (`copy.ts:1127-1129`). (b) *"priced real-flat"* **understates what IS modelled** — `escalateQuote` climbs with the
   ...
   **do NOT borrow `verdictResidualTail`'s "held flat in today's dollars"** — verbatim it is a NEW false
   ...
   The shipped sibling `recDiscAcaSlcsp` (`copy.ts:1912-1913`) hedges bidirectionally on this exact fact and
   ...
   *"One modeling choice: these prices step up with your ages, not with the way plan prices themselves climb
   ...
   blank links; mutation-proven against the shipped record. And `copy.ts:1131/1040` no longer list the
   ...
   (gated on `statePriced` alone — `copy.ts:1123-1125`). Unfiled: `shadowRateHeadroom` (`copy.ts:2641`)
...
3. **A household outside {NC, PA, FL} gets a confident winner computed with zero state income tax.**
   ...
   **His scope call.** ⚑ Two corrections from the 2026-08-02 audit: (a) the honest-withhold precedent it
   ...
   `copy.ts:1179` "State income tax isn't priced yet…"). ✅ **The GATE gap this block named is CLOSED
   2026-09-11 (Card 4):** the residual still rides `medicarePricedNote` (`healthSheetChrome.ts:454-459` —
...
5. **Smaller, each self-contained** *(all four re-anchored by the 2026-08-02 audit)*:
   ...
   - **Account balances have no magnitude sanity rule** while spend and PIA each got one (real range
     ...
     **one confirm on the household TOTAL** at the accounts step (the figure the engine actually consumes),
     reusing the running total already rendered at `copy.ts:2058` / `questions.tsx:1052-1054`.
   ...
   - **Long-term care is neither modeled nor in the OUT-but-disclosed list.** ⚑ Recommended home: a new
     ...
     **NIIT is not homeless** — `recommendationView.ts:90` emits it on *every* committed recommendation
     ...
     one-honest-home-per-fact law (`healthSheetChrome.ts:451`) forbids a silent third.
     ...
     `copy.ts:1252-1263` (keys `:1264-1405`); the panel fit arm is `vertical-fit.spec.ts:1147-1183`;
     ...
     (`recommendationView.ts:90` unconditional; `copy.ts:1130-1131`) — the scope fork is self-resolving:
...
9. **A modest-pre-tax household is refused a withdrawal-order answer the engine could compute.**
    ...
    near-term move was to reword `copy.ts:1694-1695` so it blames only the **conversion** half rather than "a
    ...
    deliberately — `copy.ts:1692-1693` records that naming "a withdrawal strategy" cures the panel's
    ...
    `copy.ts:1694-1695` with its three-rewrite comment `:1671-1693` (`:1549-1553` now holds an unrelated
    save-refusal block).
...
12. **The couple's own data.** ✅ **The warn half SHIPPED 2026-09-03** (the review's residuals sit in the
    ...
    `copy.ts:2545-2547` under the NUMBER AGREEMENT post-mortem at `copy.ts:2531`, covered by `copyGuard.test.ts:637`):
...
16. **The gates that don't bite (14 filed bullets — 13 distinct, the last two the same deferral filed
    ...
    all green. (`copy.ts:2936` and `caseStateCompanions.ts` were already correct — swept 2026-08-02.)
...
---
    ...
    `copy.ts:2983`; a new `recViz` prefix would red the three hedge-free arm labels — each measured to red
