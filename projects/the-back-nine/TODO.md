...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   **do NOT borrow `verdictResidualTail`'s "held flat in today's dollars"** — verbatim it is a NEW false
   ...
   (`devSeeds.ts:587/588` = 4200/4200) and **reversed (pessimistic)** when E<S, which `copy.ts:227` invites.
...
12. **The couple's own data.** ✅ **The warn half SHIPPED 2026-09-03** (the review's residuals sit in the
    ...
    caller is the dev seed planter, `devSeeds.ts:1698` — eight test/e2e files also call it).
    ...
    copy" and the two nearest register entries are owned **council** (`backlog.md:811`) and **pilot** (`:836`) — neither is a copy fix; the "three arms with
...
is **`lattice harbor cinder vellum 48 thicket`** (`devSeeds.ts:1140`). For RestoreFlow you need a
