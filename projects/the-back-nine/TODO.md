...
evening (b9-fri-1) BUILT (h)'s Cards 4 + 5 (`c43eb879`); 2026-09-12 (b9) RULED Card 4's sentence (`9965b09d`) and BUILT Card 10 (the nice-step lattice, `c77bcf24` + the rendered-band correction `62360933`; insight 130) — enter at the FIRST-NEXT-SESSION prescription in the 2026-09-12 close block (the Linux figure is now MEASURED, ~10 min), then (g) his eye on the remaining forks + the four-rungs question, then the rest of (h).
...
**FIRST NEXT SESSION (a prescription, ~10 min, its own commit):** the Linux six-glyph ink is MEASURED, not estimated — CI run 34707283988 (ubuntu, the `verify` job's `[instrument] … band/borderline tick column` lines): `$0.25M` / `$0.75M` / `$1.25M` **36.0 px** · `$0.5M` 30.0 · `$1M` 21.0 · `$0` 12.0 (Windows 38.5 / 32.1 / 22.4 / 12.9; the ≈35.9 derivation was 0.1 px off). The floor 34 therefore sits 2.0 px UNDER the Linux six-glyph and 1.9 px OVER the Windows five-glyph. Replace every "≈35.9 … EXPECTED / DERIVED, not measured / unconfirmed until CI" clause with the measurement + the run id, keeping each edit on its own line so no citation moves: `src/viz/bandGeometry.ts:42-45` (drop the "unconfirmed until this change's first Linux verify:fit run" sentence) · `src/ui/styles/tokens.css:117` · `src/ui/__tests__/twoPaneHonestyFloor.test.ts:116` · `e2e/chart-text.spec.ts:98`, `:117` (the floor arithmetic: "34 — 2.0 px under the Linux 36.0, 1.9 px over the Windows 32.1") and `:126` (the assertion message: "38.5 Windows / 36.0 Linux, both measured") · `e2e/chartTextAudit.ts:66` · `docs/backlog.md:902` / `docs/backlog.md:905` / `docs/backlog.md:1178` · the 2026-09-12 refinement in `docs/insights/118-…md` · `docs/architecture.md:328` — its "(≈35.9 × 1.5 ≈ 54)" becomes "36.0 × 1.5 = 54.0, EXACTLY the ~54 px column at a 24 px root — zero margin on Linux, which is the strongest reason the rem derivation must rest on the grid floor, not on tick ink". Then `pnpm verify:doc-stats`; if any line count moved, the re-anchor tool (this session's scratchpad `reanchor-delta.mjs --base 62360933 --apply`, ONCE).
