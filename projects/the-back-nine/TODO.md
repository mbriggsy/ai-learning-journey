...
0. **Doc restructure — GREEN-LIT, mid-flight. M1–M4 DONE.** Full ground-up rewrite. Foundation (CP1 + CP1.5), **M1 (the migration ledger)**, **M2 (the four hubs)**, **M3 (the four `decisions/` records)**, and **M4 (the four plans)** are committed. M4 folded the SS sub-engine into plan-1 as first-class build content + dropped the C-unit "fold" framing, folded the R40 5-unit + portfolio multi-holding builds into plan-2 (pointing into the `decisions/` records), and stripped plan-3/plan-4 changelogs; 318/318 links resolve. **Resume at M5 — rewrite the two research docs** (`research/engine-validation-and-tax.md` + `research/pre65-healthcare.md`) present-tense from the ledger's `## docs/research/*` home-sections: strip the history appendix, KEEP the live strands + the `verify:aca` gate + the `[CORRECTED]` markers, and confirm the crypto-stack comparative rationale reads present-tense in research (the M3-decided home). Read [`RESTRUCTURE-PLAN.md`](RESTRUCTURE-PLAN.md) §"RESUME HERE" for the carry-forward notes. Do NOT delete `features/` or strip history until M6's zero-loss gate. Delete `RESTRUCTURE-PLAN.md` + `.recovery/` when it lands.
