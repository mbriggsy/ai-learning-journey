...
`copy.ts:2683`-at-filing → `copy.ts:2926` (`recDeltaVizAria`) + `recommendationView.ts:681-687` → `:697-703` (u17 saved-rec build
spec) and `copy.ts:2682`-at-filing → `copy.ts:2926` (this file's item-10 entry; the "-at-filing" numbers are the RECORDED stale
...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   + `:1731-1738` → `healthOverlay.ts:270`. (b) `copy.ts:1131` is a Medicare eyebrow; the strings that claim
   ...
   both lists (`copy.ts:1084-1097`); what is genuinely unmodelled about it is the COST TREND (`copy.ts:1099-1101`). (b) *"priced real-flat"* **understates what IS modelled** — `escalateQuote` climbs with the
   ...
   **do NOT borrow `verdictResidualTail`'s "held flat in today's dollars"** — verbatim it is a NEW false
   ...
   The shipped sibling `recDiscAcaSlcsp` (`copy.ts:1868-1869`) hedges bidirectionally on this exact fact and
   ...
   *"One modeling choice: these prices step up with your ages, not with the way plan prices themselves climb
   ...
   blank links; mutation-proven against the shipped record. And `copy.ts:1103/1040` no longer list the
   ...
   (gated on `statePriced` alone — `copy.ts:1095-1097`). Unfiled: `shadowRateHeadroom` (`copy.ts:2593`)
...
3. **A household outside {NC, PA, FL} gets a confident winner computed with zero state income tax.**
   ...
   **His scope call.** ⚑ Two corrections from the 2026-08-02 audit: (a) the honest-withhold precedent it
   ...
   `copy.ts:1151` "State income tax isn't priced yet…"). ✅ **The GATE gap this block named is CLOSED
...
5. **Smaller, each self-contained** *(all four re-anchored by the 2026-08-02 audit)*:
   ...
   - **Account balances have no magnitude sanity rule** while spend and PIA each got one (real range
     ...
     **one confirm on the household TOTAL** at the accounts step (the figure the engine actually consumes),
     reusing the running total already rendered at `copy.ts:2015` / `questions.tsx:1052-1054`.
   ...
   - **Long-term care is neither modeled nor in the OUT-but-disclosed list.** ⚑ Recommended home: a new
     ...
     **NIIT is not homeless** — `recommendationView.ts:90` emits it on *every* committed recommendation
     ...
     `copy.ts:1224-1235` (keys `:1236-1361`); the panel fit arm is `vertical-fit.spec.ts:1147-1183`;
     ...
     (`recommendationView.ts:90` unconditional; `copy.ts:1102-1103`) — the scope fork is self-resolving:
...
9. **A modest-pre-tax household is refused a withdrawal-order answer the engine could compute.**
    ...
    near-term move was to reword `copy.ts:1650-1651` so it blames only the **conversion** half rather than "a
    ...
    deliberately — `copy.ts:1648-1649` records that naming "a withdrawal strategy" cures the panel's
    ...
    `copy.ts:1650-1651` with its three-rewrite comment `:1627-1649` (`:1505-1509` now holds an unrelated
    save-refusal block).
...
12. **The couple's own data.** ✅ **The warn half SHIPPED 2026-09-03** (the review's residuals sit in the
    ...
    copy" and the two nearest register entries are owned **council** (`backlog.md:789`) and **pilot** (`:835`) — neither is a copy fix; the "three arms with
    ...
    `copy.ts:2497-2499` under the NUMBER AGREEMENT post-mortem at `copy.ts:2483`, covered by `copyGuard.test.ts:637`):
...
16. **The gates that don't bite (14 filed bullets — 13 distinct, the last two the same deferral filed
    ...
    all green. (`copy.ts:2879` and `caseStateCompanions.ts` were already correct — swept 2026-08-02.)
...
---
    ...
    `copy.ts:2926`; a new `recViz` prefix would red the three hedge-free arm labels — each measured to red
