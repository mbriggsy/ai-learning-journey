...
0. **Doc restructure — GREEN-LIT, mid-flight. M1 + M2 + M3 DONE.** Full ground-up rewrite. Foundation (CP1 + CP1.5), **M1 (the migration ledger)**, **M2 (the four hubs)**, and **M3 (the four `decisions/` records)** are committed. M3 created `ss-computation` / `other-income-r40` / `portfolio-holdings` and reframed `accumulation-fuck-off-date` present-tense (stripped its changelog), linkified the three M2 inline-code refs, and repointed `decisions/README`; §1014 + no-tax-blind-arm rehomed per the ledger, crypto-stack left in research, 312/312 links resolve. **Resume at M4 — rewrite the plans** (`plans/1-engine` … `plans/4-recommendation`) present-tense from the ledger's `## docs/plans/*` home-sections: C1–C3 first-class in plan-1, R40 + portfolio build into plan-2, right-size plan-3/4 — and make the plans **point into** the `decisions/` records for the WHY (so M6's demolition of `features/` leaves no dangling reference). Read [`RESTRUCTURE-PLAN.md`](RESTRUCTURE-PLAN.md) §"RESUME HERE" for the exact spots + carry-forward notes. Do NOT delete `features/` or strip history until M6's zero-loss gate. Delete `RESTRUCTURE-PLAN.md` + `.recovery/` when it lands.
