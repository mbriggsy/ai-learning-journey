...
**The phone strip-reserve fork** (register *The phone intake walk* item 3 — one of the HIS EYE batch's three councils): the two-block strip measured 278 px =
...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   (a) `healthOverlay.ts:296` is a **closing brace**, not a consumer; the real seam is `taxOverlay.ts:1698` plus `taxOverlay.ts:1740-1747`
   → `healthOverlay.ts:270`. (b) `copy.ts:1171` is a Medicare eyebrow; the strings that claim
...
5. **Smaller, each self-contained** *(all four re-anchored by the 2026-08-02 audit)*:
   ...
   - **Long-term care is neither modeled nor in the OUT-but-disclosed list.** ⚑ Recommended home: a new
     ...
     and the fit gate's panel arm (`vertical-fit.spec.ts:1687-1716`) asserts only that the dialog box fits
     ...
     **NIIT is not homeless** — `recommendationView.ts:90` emits it on *every* committed recommendation
     ...
     `copy.ts:1264-1275` (keys `:1276-1417`); the panel fit arm is `vertical-fit.spec.ts:1687-1716`;
...
13. **Also:** ✅ icons + an installable PWA shipped 2026-09-08 · WebKit reaches only the VAULT seam (2026-09-08 — the two
    ...
    rules; the `newCDPSession` pattern is at `caddie-walk.spec.ts:318`, and the shipped arm's own helper is `raiseDefaultFont` (`vertical-fit.spec.ts:465-478`); re-run the 8 REAL+TIER one-frame
...
- **`verify:fit` does NOT measure the recommendation surface** (`e2e/vertical-fit.spec.ts:704-714`
  excludes the committed + held renders — a live solve blows the fit harness's 180 s per-test budget, `playwright.fit.config.ts:53`). So *"seat it and re-measure
  under `verify:fit`"* is **unexecutable** for anything in `.rec-committed__rest`; it needs a MANUAL
  1536×791 measure. And the *"~89px headroom"* number is the SPINE idle frame (`vertical-fit.spec.ts:2361-2363`), a once-measured
...
`src/ui/devSeeds.ts` — `DEV_SEEDS` at `devSeeds.ts:1121`, `AGED_PLANTS` at `devSeeds.ts:1665`.
...
| `buckets` | **the ordering witness** — 3 real buckets (pre-tax + taxable-with-gain + Roth) on the `proportional` default, so `taxable-first` and the household's own order finally DIVERGE. The only seed on which "your plan today" is observably their plan. Live solve (~25 min in dev, measured 2026-08-03 — the single-bucket `nc` measured ~11) |
