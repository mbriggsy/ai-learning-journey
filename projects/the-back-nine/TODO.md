...
2. **Pre-65 ACA premiums are priced real-flat — the sin the Medicare council ruled solver-BLOCKING.**
   ...
   (a) `healthOverlay.ts:296` is a **closing brace**, not a consumer; the real seam is `taxOverlay.ts:1695`
   ...
   schedule build to an engine-owned `buildAcaPricingSchedule` beside `partBPricingByT` (`taxOverlay.ts:1116`).
   ...
   **do NOT borrow `verdictResidualTail`'s "held flat in today's dollars"** — verbatim it is a NEW false
   ...
   (`devSeeds.ts:611/588` = 4200/4200) and **reversed (pessimistic)** when E<S, which `copy.ts:227` invites.
...
3. **A household outside {NC, PA, FL} gets a confident winner computed with zero state income tax.**
   ...
   **His scope call.** ⚑ Two corrections from the 2026-08-02 audit: (a) the honest-withhold precedent it
   ...
   Pricing is membership-keyed at `taxOverlay.ts:873`; `PRICED_STATES` is `constants/stateTax.ts:50`; the
...
5. **Smaller, each self-contained** *(all four re-anchored by the 2026-08-02 audit)*:
   ...
   - **Long-term care is neither modeled nor in the OUT-but-disclosed list.** ⚑ Recommended home: a new
     ...
     **and** `scrollHeight > clientHeight` — content growth makes the second assertion *more* true.
     ...
     put" / "simply sits."** The balance is not parked, it is **destroyed**: `taxOverlay.ts:1818-1819` sets
     ...
     fix-or-it-re-rots) **and** `taxOverlay.ts:1809-1811`, which still calls post-65 HSA-as-ordinary-income
     ...
     **NIIT is not homeless** — `recommendationView.ts:90` emits it on *every* committed recommendation
     ...
     `src/engine/taxOverlay.ts:1808-1811` (there is no `overlays/` dir). NIIT's two homes confirmed
     ...
     branches (`taxOverlay.ts:1817-1823`, `:1837-1842`, `:1965-1968`): on each, EVERY bucket is zeroed
...
12. **The couple's own data.** ✅ **The warn half SHIPPED 2026-09-03** (the review's residuals sit in the
    ...
    caller is the dev seed planter, `devSeeds.ts:1722` — eight test/e2e files also call it).
    ...
    copy" and the two nearest register entries are owned **council** (`backlog.md:813`) and **pilot** (`:838`) — neither is a copy fix; the "three arms with
...
`src/ui/devSeeds.ts` — `DEV_SEEDS` at `:1076`, `AGED_PLANTS` at `:1618`.
...
is **`lattice harbor cinder vellum 48 thicket`** (`devSeeds.ts:1164`). For RestoreFlow you need a
