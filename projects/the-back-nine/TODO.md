...
**NEXT, in order (every ruling is `execute`; nothing below waits on him — the fresh session enters at 1):**
1. **healthnc's Medicare-era premium sentence** (Tier 0, NEW — the walk's calm-but-wrong: "~$2,700 a year for your household" is ONE enrollee's first year on a 65/63 couple that pays ~$5,700 once both are on Medicare): red-first in `healthSheetChrome.test` on the 65/63 plant; the prescription is in the entry; `retired` byte-untouched.
2. **Card 13's (iv)** — the ladder readout shows the CROWN's line at rest (`readoutIdx = scrubIdx ?? crown index`; `data-active` on it; the scrub rule + `scrubbed` stay gated on `scrubIdx`; no CSS; the readout stays `aria-hidden`; amend `OddsLadder.test.tsx`) + fuckOffDate.css's D2d comment (it describes candidate (i) as shipped — both graphs live in `.fod-graphs`) + the design skill's "on-demand, not on the first frame" clause (architecture §12 wins) + the `h2.fod-headline` aria-describedby arm on the REAL date arms; THEN the REAL catalog capture (date65 / datenc / datesplit + one live-Chrome scrollbar look) before the column half files durably.
3. **Card 6's three chart-text arms** — the register's CORRECTED mechanism (2026-09-13): the shipped stagger already seats named moments first; the build is a crossing-aware stagger — exhaustive assignment over the named items (≤ 5): min rows → min tail crossings (an item's x inside a HIGHER row's box) → min Σ rank×row (`data-ct-rank`: crown / work-stops 3, today 2, built + horizon 1) → the greedy order as tiebreak; unnamed ticks unchanged (row 0 if clear, else hidden); `.ct-block__name` min-height one line so an unnamed item's ages sit on the AGE baseline (an empty block has zero height today — the cause of (c)). Predicted `?vault=datestale` PHONE: Work stops / Today / Plan built on rows 0 / 1 / 2, zero crossings, still 3 rows (AGED_DATE_ROWS holds). Arms (chart-text.spec.ts, datestale PHONE + FLOOR, each red today): tail-vs-box zero crossings · the top-ranked named moment on row 0 · unnamed sub baseline == a named sub baseline on its row.
4. **Card 2's lever (ii)** — `composeReadoutLines` (`src/viz/bandData.ts`): a degenerate range (`row.low === row.high`) composes ages / a "land at" label (new BandLabels key ← `bandReadoutRangeSameLabel`) / ONE figure, dropping the median pair (the same string); unit + the chart-text gate's band readout arm; `BAND_SEAT.PHONE` MEASURED, never decreed. The four-rungs EXPIRY watches lever (iii) — the p50 figure at rest in the laptop seat.
5. **The four-rungs deliverables** — the pbt's RECORDED-LIMIT docblock ([1001,1250] are SHIPPED mislabelled ladders; the k-lock rung moves that arm DOWNWARD, red-first); the direction-neutral `assertTickColumn` arm (a crowding FLOOR from the measured 16.9 px line box on both platforms; `.band-tick` count === intervals + 1) + the min/max adjacent-gap EMITTER as recorded distances (insight 065); no max-gap threshold. The CLOSED *Two dollar ladders* entry's RULED bullet carries it.
6. **Card 11's (c)** — measure the hero's ink at the widest catalog delta string on the 358 px host in a real browser, then name + pin `HERO_HALF_RESERVE` (`RV_PLOT.left + plotW + reserve ≤ RV_VIEW.w`); "~151u" is the council's estimate.
7. **The assumptions row's "never oversold"** (Tier 1, NEW) · **the family's other two primaries** (Tier 2, NEW: BudgetBuilder's reason-less blocked Apply; SequencingControl's live no-op on the applied pick).
8. **Card 1** — delete the two legend outlines (`band.css` `.band-legend__sw--inner/--outer`) + rewrite the two comments + the a11y gate (the legend's fills source-bound to the chart's `BAND_FILL_*`; band.css has no outline on the swatches) · **Card 3** (the words: my lean = the register's draft, both endpoints in-sentence) + the plumbing · **Card 9** (don't show the picker to the already-short cohort; the invite opens the refusal with one line) · the copy forks (my leans as filed; the Roth sheet's three added 2026-09-13) — each Caddie-read before ship.
9. Councils still owed: the 20 px two-pane fork · the phone strip-reserve fork · the lever sheets' hidden actions row + Apply-before-the-comparison (Tier 3, NEW — his or a council). Then a FADING-seed walk (the legend reopen trigger has no referent on any fresh seed) · the forced-colors audit of `src/viz` (Tier 4, NEW) · the Tier 3 omitted-costs discussion (needs him) · the NIIT gloss (his).
**HIS EYE (unchanged, plus this session's):** Card 1 / Card 2 (iii) / Card 9's framing / the two ⚑ composition sign-offs (the RV label gutter at 52.3 % of plot run; the date route's bare left column) / the four-rungs taste sign-off (four stays) / the 24 px + 20 px frames / the icon / the NEW Tier 3 sheets entry (the hidden actions row at his seat). Frames: `temp/caddie/2026-09-13-eye/INDEX.md` · `temp/caddie/2026-09-13-sheets/look/` (both sheets, blocked / pointed / cleared, both seats).
