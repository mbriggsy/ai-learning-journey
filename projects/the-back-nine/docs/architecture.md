...
- **The containment premise is RATIFIED and additive** (the U9a-medical-fence class question, closed): the household's entered spending figure **EXCLUDES the Medicare Part B premium and its IRMAA surcharge — the tool adds them on top** (`fundingNet = net + medicareCost`), exactly as it already excluded ACA premiums; OOP medical (and Part D / Medigap premiums, which the engine deliberately does not price) live **INSIDE** the entered spending. The intake spend copy carries this boundary in words (`spendHelp`), and SS stays **gross** (the FRA statement figure — no premium netting anywhere on the income side). Omission was the optimistic cardinal sin; a user-side double-entry is pessimistic-safe (insight 055's composition).
- **Part B is priced REAL-FLAT (no trend) — a disclosed modeling choice and an Act-4 SOLVER-BLOCKING forward landmine.** The base premium is a constant real dollar (`partB2026`); reality trends Part B faster than CPI, and real-flat specifically **under-penalizes IRMAA cliff-CROSSING** in later years. The residual disclosure names it ("held flat in today's dollars… could sit a little tighter than shown"). Before the Act-4 solver ranks conversion strategies whose payoff rides late-year IRMAA crossings, a **sourced** Medicare-cost-trend constant must land (never a guessed growth rate — burned/062); until then the honest contract is the disclosed real-flat model. A dated tripwire (`irmaaTopTierReindex.tripwire.test.ts`) separately forces a top-tier re-verify at the 2028 re-index.
