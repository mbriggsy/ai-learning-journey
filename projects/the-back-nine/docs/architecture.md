...
- **The svg holds geometry only.** Fan, median, gridlines, annotation rules, dots, bars, markers, leaders, the scrub rule and the transparent capture rect. A `<text>` inside a chart svg is a regression (`ConfidenceBand.test.tsx`, `OddsLadder.test.tsx`, `TwoFutures.test.tsx` and the e2e gate all pin `svg text` count 0).
...
- **The gate is real Chromium** (`e2e/chart-text.spec.ts`, on the fit harness, CI-enforced through `verify:fit`): on PHONE / a 320 reflow arm / FLOOR / REAL, every visible chart text node renders at ≥ `--text-xs` (read from the stylesheet), lies inside its card or column, overprints nothing, no named label is hidden, the text grows when the reader raises their browser font, and reduced motion renders the same nodes — each oracle with a planted clip / planted shrink as its control.
