...
- **Deemed filing:** a **single `claimAge` per person drives BOTH own and spousal** (no restricted application — both modeled cohorts are post-1954, fully subject). It is **structural, not a runtime check**: `PersonInputs` carries exactly one `socialSecurityClaimAge` (`model.ts:112`) and `householdBenefits` derives both pieces from it, so a second claim age is unrepresentable and there is nothing a test could catch. The only deemed-filing test pins the cutoff DATE (`constants.shape.test.ts:747-749`). The **survivor branch is the lone independent-timing exception**.
