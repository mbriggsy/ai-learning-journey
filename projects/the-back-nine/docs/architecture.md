...
- **The deduction stack carries dated, sunsetting provisions.** Standard deduction + the age-65 additions (§63(f), keyed off the biological `count65`) + the **OBBBA-2025 senior bonus** with its MAGI phase-out. The senior bonus is available **tax years 2025–2028 and sunsets after 2028** unless extended — its constants entry carries the sunset marker (§8). **KNOWN GAP (U13 ultramode, 2026-07-09 — filed REQUIRED, build-tripwired):** the engine does not yet READ that marker — `seniorBonusFor` has no year parameter, so every sim year credits the bonus, an optimistic mispricing for calendar years 2029+. The filed engine sunset unit (thread the sim-year calendar through `deductionStack`/`seniorBonusFor` + the magiLandscape rails; DND-012 external fixtures; council-ratified) must land before tax-year 2028 closes — `seniorBonusSunset.tripwire.test.ts` fails every build from 2028-01-01 until it does. The staleness layer deliberately carries NO sunset-crossing note (the crossing changes nothing about a saved answer once the engine models it — `staleness.ts` header has the law).
...
- **Dated, sunsetting provisions carry a sunset marker — and the ENGINE must consume it.** A figure scheduled to expire (e.g. the OBBBA-2025 **Senior Bonus Deduction**, tax years 2025–2028) is year-keyed with its legal basis **and** an explicit sunset year, so a bracket/provision change reads as a vintage bump rather than silent drift. The honesty obligation of R14/R22 lands **in the pricing, not in a note** (U13 ultramode, 2026-07-09): a "calm note at the sunset crossing" was refuted — the crossing changes nothing about a correctly-priced saved answer, and a note over an UN-modeled sunset asserts a re-pricing that never happened. The senior-bonus engine consumption is the filed REQUIRED unit (§7.2 gap note); the sunset tripwire test enforces the deadline. The RMD age is the same shape done right — a per-person function of birth year (72 / 73 / 75, with the legislated 2033 step) the engine consumes directly, never a hardcoded literal.
