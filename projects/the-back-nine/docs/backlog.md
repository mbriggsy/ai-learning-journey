...
### The Social Security taxation thresholds are FROZEN by law and applied FLAT in real dollars — the engine indexes what Congress froze, so every later year taxes less of a benefit than the law will
...
- ⚑ **RE-SEVERITY 2026-08-03 (verification fleet) — this is a three-tap path to a wrong plan, not a copy
  ...
  **`copy.ts:1365-1366`**. The panel toggle (`AssumptionPanel.tsx:478-481`) writes `spendEntryPeriod`
...
- ⚑ **A first replacement draft was written and REJECTED — do not re-derive it.** It dropped the live
  ...
  the now-overbroad comments at `copy.ts:1339-1341` and `AssumptionPanel.tsx:244-245`
...
- ✅ **CLOSED 2026-08-03 (`bd851f24`)** — the card asserted "It still matches your plan as it stands today." while the door beneath it showed the household never executed the advice; the holds line is now `recommendRecordHolds` = "It still lines up with the numbers you've entered." (`copy.ts:1602`, the defending comment `copy.ts:1572-1601`). *The live half of this entry is the strategy-naming ruling below.*
...
- ⚑ **`?vault=rec` CANNOT WITNESS this defect.** `devSeeds.ts:857-859` → `retiredOnTrack` holds **one**
  ...
  from `S` to `M`.
- ⚑ Anchor drift: `noChange: false` is `devSeeds.ts:1479` (not `:1322`); the winner literal is `:1465`
  (not `:1308`). `copy.ts:1566-1570` is the card-FAMILY header — the holds sentence's defending comment is
  `copy.ts:1572-1601` (added 2026-08-03), and it already carries the ⚠️ NEVER DEFEND THIS LINE WITH `noChange` warning.
...
- ⚑ **RETITLED 2026-08-02 — the filed claim ("cannot be seen") is HALF FALSE and the truth is worse.**
  It IS disclosed: `recommendationView.ts:88-91` → `copy.ts:2749-2750` → `RecommendationSurface.tsx:477-487`.
...
- ✅ **(XS) SHIPPED 2026-08-02 — the dead-end clause is GONE.** `copy.ts:2749-2750` now states the
  ...
  `copy.ts:2941-2942` under the comment at `:2926-2935` that states the both-ways coupling.
...
- **Near-term, honest, XS:** fix `copy.ts:1694-1695` only — it says a *withdrawal strategy* needs more pre-tax when only the **conversion** half does. Sequencing across taxable and Roth is real and rankable.
...
- **STILL OPEN — R23: the runner-up is retained but never IDENTIFIED**, and its `why` is one
  content-free constant (`copy.ts:1865-1866`) naming neither arm, while `payload.runnerUp.id`/`.policy` sit
...
### The premium card's two era figures scale by a law the card never states — ~$8,100 is 3× ~$2,700 while the extras pair on the same card is 2×, the shape he once read as "test data issue?"
...
- **🟡 "Still being finalized" reads as a PROGRESS state and is a PERMANENT disclosure — and it is
  ...
  `?vault=rec` payload carries `disclosedDirectional: []` (`devSeeds.ts:1488`), so the plants HIDE a
