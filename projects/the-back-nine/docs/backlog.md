...
- ⚑ **`?vault=rec` CANNOT WITNESS this defect.** `devSeeds.ts:881-883` → `retiredOnTrack` holds **one**
  ...
  from `S` to `M`.
- ⚑ Anchor drift: `noChange: false` is `devSeeds.ts:1503` (not `:1322`); the winner literal is `:1489`
...
- **Edge accepted:** a rail-anchored amount landing EXACTLY on the post-RMD headroom would make the
  +$1,000 variant cap to the SAME conversion (`taxOverlay.ts:1450` — `min(planned, pretax − rmd)`, the
...
- **🟡 "Still being finalized" reads as a PROGRESS state and is a PERMANENT disclosure — and it is
  ...
  `?vault=rec` payload carries `disclosedDirectional: []` (`devSeeds.ts:1512`), so the plants HIDE a
...
### The IRMAA top tier is frozen through 2027 and the pricing does not know it — `topTierFrozenThrough` reaches the disclosure map and no consumer, so 2027's top line sits ~3 % too high in real dollars

`S` · **pilot** · filed 2026-09-24 by the SS-thresholds build's trace (the same frozen-nominal-in-a-real-engine class, one tier, one year)

- `irmaa.value.topTierFrozenThrough` (`src/engine/constants/health.ts`, the IRMAA schedule: `2027`; its note: "the top tier (≥ $500k single / ≥ $750k MFJ) is frozen through 2027, re-indexes 2028") is exported into the disclosure map (`healthConstants.irmaaTopTierFrozenThrough`) and read by NOTHING that prices — the surcharge walk (`healthOverlay.ts`, the per-tier loop over `schedule.tiers`) compares the two-year-lookback MAGI, carried in REAL dollars, against every tier's NOMINAL threshold held flat, which is correct for tiers 1–4 (indexed annually by law — flat-real IS the law, the same reason the brackets stay flat) and wrong for the top tier in calendar 2027 only: its $750k MFJ / $500k single line is worth $750k ÷ 1.032 ≈ $726.7k real that year, so a household whose bill-setting MAGI lands between ~$726.7k and $750k (2025 income for the 2027 bill) is charged tier 4 where the law charges tier 5 — ~$40.70 + $7.70 a month per person (Part B 487.0 − 446.3, Part D 91.0 − 83.3), one year, top-decile households. Rosy, bounded, and by construction absent from every shipped seed (no seed's MAGI approaches $700k). Filed for the record, not the queue.
- **Prescription:** in the tier walk, deflate the TOP tier's nominal threshold by `cumulativePriceIndex(min(calendarYear, topTierFrozenThrough))` (`src/engine/priceIndex.ts` — the one index; for 2028+ the statute re-indexes from the 2027 base, so the real line HOLDS at its 2027-deflated level, never climbs back); tiers 1–4 untouched. Red-first: a household with a $740k bill-setting MAGI pays tier 4 in bill-year 2026, tier 5 in 2027 and 2028; the reduce-to-spine goldens untouched (healthcare-off never enters the walk).
- ⚑ NEGATIVE: never deflate tiers 1–4 (they index by law); never mint a second index or a hand rate (CRN, one path); never treat "re-indexes 2028" as a return to the $750k real line (the re-index base is the frozen 2027 nominal).
