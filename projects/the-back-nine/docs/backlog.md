...
1. ✅ **SHIPPED 2026-09-05 — SVG DRAWS, HTML WRITES (council wf_ecbe0ab2-7bb, 8/10; `docs/architecture.md §12`; insights 115 + 116).**
   ...
   worst case (RE-MEASURED 2026-09-12, Card 10: `borderline`'s six-glyph `$0.25M` / `$0.75M` / `$1.25M`, 38.5px on Windows / ≈35.9px
   EXPECTED on Linux CI — FreeType rounds glyph advances to whole pixels; the seven-glyph `$0.375M` at 45.0 / 42.0 is unproducible since the nice-step lattice) hangs
   2.9px left of the 238px figure into the drawer's 25px chrome, 22.1px clear of the card edge (`assertTickColumn` reds it
   inside 4px; its seed guard pins the six-glyph dollar by TEXT — `/^\$\d\.\d{2}M$/` — and floors the ink at 34, between the five-glyph Windows 32.1 it must exclude and the six-glyph Linux ≈35.9 it must pass); the ladder's "on track" (44.3px against a 43.2px column on a 288px figure) takes ~1px of the
