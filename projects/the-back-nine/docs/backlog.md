...
- ⚑ **RE-SEVERITY 2026-08-03 (verification fleet) — this is a three-tap path to a wrong plan, not a copy
  ...
  **`copy.ts:1347-1348`**. The panel toggle (`AssumptionPanel.tsx:478-481`) writes `spendEntryPeriod`
...
- ⚑ **A first replacement draft was written and REJECTED — do not re-derive it.** It dropped the live
  ...
  the now-overbroad comments at `copy.ts:1321-1323` and `AssumptionPanel.tsx:244-245`
...
- ✅ **CLOSED 2026-08-03 (`bd851f24`)** — the card asserted "It still matches your plan as it stands today." while the door beneath it showed the household never executed the advice; the holds line is now `recommendRecordHolds` = "It still lines up with the numbers you've entered." (`copy.ts:1568`, the defending comment `copy.ts:1538-1567`). *The live half of this entry is the strategy-naming ruling below.*
...
- ⚑ Anchor drift: `noChange: false` is `devSeeds.ts:1479` (not `:1322`); the winner literal is `:1465`
  (not `:1308`). `copy.ts:1532-1536` is the card-FAMILY header — the holds sentence's defending comment is
  `copy.ts:1538-1567` (added 2026-08-03), and it already carries the ⚠️ NEVER DEFEND THIS LINE WITH `noChange` warning.
...
- ⚠️ Anchor corrections: `healthOverlay.ts:296` is a **closing brace**, not a consumer (real seam `taxOverlay.ts:1695` + `:1737-1744` → `healthOverlay.ts:270`); the strings claiming the coupling is priced are `copy.ts:989-992`, not `copy.ts:933`.
- ⚠️ **Re-tag: BLOCKED ON RESEARCH.** No sourced ACA cost-trend primary exists in the repo, so a solver block would hold for months over the whole pre-65 population. Near-term move is the copy swap at `copy.ts:989-992`. The excess-APTC field moved to `aca-last-verified.json:43` and `scripts/verify-aca-status.ts:40-103` declares and REQUIRES the key (`adjacentButSharp` at `:90`, `needProse` at `:169`, since 2026-08-03) — **but the gate is presence-only: nothing checks that the prose models the uncapped clawback.**
...
### The assumptions panel's conversion row says the lever's benefit "reads understated, never oversold" while the Roth sheet one door over discloses an optimistic modeling choice

`S` · **pilot (the sentence) · briggsy (the words)** · filed 2026-09-13 by the sibling-sheets Caddie walk (Card 7 — the false-pass hunter on `seed:retired`, HARD-FLAG, lane both; survived with its premise trimmed: the NIIT leg was fabricated, the state-tax leg is disclosed inside the sheet)
...
- ⚑ **RETITLED 2026-08-02 — the filed claim ("cannot be seen") is HALF FALSE and the truth is worse.**
  It IS disclosed: `recommendationView.ts:88-91` → `copy.ts:2707-2708` → `RecommendationSurface.tsx:477-487`.
...
- ✅ **(XS) SHIPPED 2026-08-02 — the dead-end clause is GONE.** `copy.ts:2707-2708` now states the
  ...
  `copy.ts:2899-2900` under the comment at `:2884-2893` that states the both-ways coupling.
...
- **Near-term, honest, XS:** fix `copy.ts:1660-1661` only — it says a *withdrawal strategy* needs more pre-tax when only the **conversion** half does. Sequencing across taxable and Roth is real and rankable.
...
- **STILL OPEN — R23: the runner-up is retained but never IDENTIFIED**, and its `why` is one
  content-free constant (`copy.ts:1831-1832`) naming neither arm, while `payload.runnerUp.id`/`.policy` sit
...
- **🟡 Error copy speaks to the absurd reading:** birth year "62" → "Ages past 119 are beyond what
  ...
  `ACCOUNT_KINDS` `model.ts:1439-1447` + labels `copy.ts:297-303` · `copy.accountSave` rendered
