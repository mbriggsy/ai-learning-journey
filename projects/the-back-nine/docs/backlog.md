...
- **Prescription:** the row must carry the same bounded claim the sheet does — *"…errs against converting on every rule it prices, and one modeling choice (the surcharge's drug-plan piece past 2035) leans the other way — see the Roth sheet."* (the pilot's draft; his words). Pin: a unit test on the assumptions composer asserting the row never contains "never oversold" while `medicare-cost-trend`'s post-edge hold is the shipped rule; the sheet's disclosure sentence and the row's clause read ONE constant.
...
- **BudgetBuilder** (door 2, an empty budget): *"Use this budget"* renders as a grey underlined link twinned with *Cancel* — same shape, weight and underline; only tint says one cannot commit — and NO reason renders at either viewport (aria: `[disabled]`; `dialog.txt` / `copy.txt` / `sr-only.txt` carry none). The Card 14a class on a FOURTH sheet. CONTEXT, not a kill: Card 9 (2026-08-02) deliberately flipped the empty state's emphasis to "Add a line" and demoted the Apply to quiet — the ruling covers the emphasis, not a reason-less tint-only block; `budgetApplyEmpty` exists as a key and the blocked press may already speak it (open the lines: `BudgetBuilder.tsx`, `intake.css` `.budget-sheet__actions`, the `.btn-quiet[aria-disabled]` rule in save.css).
