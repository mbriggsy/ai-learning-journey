...
- ⚑ **RE-SEVERITY 2026-08-03 (verification fleet) — this is a three-tap path to a wrong plan, not a copy
  nit.** Anchor drift first: the cold-read log cites `copy.ts:1151`; the live string is
  **`copy.ts:1268-1269`**. The panel toggle (`AssumptionPanel.tsx:478-481`) writes `spendEntryPeriod`
...
- ⚑ **A first replacement draft was written and REJECTED — do not re-derive it.** It dropped the live
  ...
  the now-overbroad comments at `copy.ts:1242-1244` and `AssumptionPanel.tsx:244-245`
...
- ✅ **CLOSED 2026-08-03 (`bd851f24`)** — the card asserted "It still matches your plan as it stands today." while the door beneath it showed the household never executed the advice; the holds line is now `recommendRecordHolds` = "It still lines up with the numbers you've entered." (`copy.ts:1489`, the defending comment `copy.ts:1459-1488`). *The live half of this entry is the strategy-naming ruling below.*
...
- ⚑ Anchor drift: `noChange: false` is `devSeeds.ts:1457` (not `:1322`); the winner literal is `:1443`
  (not `:1308`). `copy.ts:1453-1457` is the card-FAMILY header — the holds sentence's defending comment is
  `copy.ts:1459-1488` (added 2026-08-03), and it already carries the ⚠️ NEVER DEFEND THIS LINE WITH `noChange` warning.
...
- **Near-term, honest, XS:** fix `copy.ts:1581-1582` only — it says a *withdrawal strategy* needs more pre-tax when only the **conversion** half does. Sequencing across taxable and Roth is real and rankable.
...
- **STILL OPEN — R23: the runner-up is retained but never IDENTIFIED**, and its `why` is one
  content-free constant (`copy.ts:1743-1744`) naming neither arm, while `payload.runnerUp.id`/`.policy` sit
