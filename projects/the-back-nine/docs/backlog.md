...
### The health sheet's Medicare-era premium sentence quotes ONE enrollee's first year as the household's whole era — "~$2,700 a year for your household" on a couple that pays ~$5,700 once both are enrolled

`S` · **pilot** · filed 2026-09-13 by the sibling-sheets Caddie walk (Card 8 — the false-pass hunter on `seed:healthnc`, BLOCKER, lane both; two refuters survived it, one killed only the unit reading)

- *"In your plan's Medicare years, premiums could run about ~$2,700 a year for your household"* (`healthSheetChrome.ts`, the premium fact card): `medicareAnchor` takes the FIRST readout year anyone is enrolled, and on the 65/63 household that year holds ONE enrollee — so the figure is the household's true total for THAT year and false for the era the sentence names (two years on both are enrolled: roughly double, plus the extras). The step card directly beneath says *"could add about ~$1,100 a year for each of you once on Medicare"* — the same-named quantity in a different enrollment frame on one screen, and nothing adjacent (nor door 6) says who is enrolled when. Rosier than true on the number a couple repeats aloud — the named sin.
- **Prescription:** speak the era honestly — either anchor the premium sentence at the first year BOTH are enrolled (the last enrollment onset) when the household has one, or keep the first-enrollment anchor and SAY whose year it is (*"…about ~$2,700 a year once Alex is on Medicare, and about ~$5,700 once you both are"*); the two cards must share one enrollment frame. Red-first on `?seed=healthnc` (the 65/63 plant) in `healthSheetChrome.test`; the all-65+ households (`retired`) must be byte-untouched (one anchor, both enrolled — the sentence already reads right there).
- ⚑ NEGATIVE: never average the two years into one figure (a number no year pays); never drop the household frame for a per-person one across the whole sheet (the extras and the step cards are already per-person by construction — the fix is the PREMIUM card's frame, stated).
...
- ⚑ **RE-SEVERITY 2026-08-03 (verification fleet) — this is a three-tap path to a wrong plan, not a copy
  nit.** Anchor drift first: the cold-read log cites `copy.ts:1181`; the live string is
  **`copy.ts:1298-1299`**. The panel toggle (`AssumptionPanel.tsx:478-481`) writes `spendEntryPeriod`
...
- ⚑ **A first replacement draft was written and REJECTED — do not re-derive it.** It dropped the live
  ...
  the now-overbroad comments at `copy.ts:1272-1274` and `AssumptionPanel.tsx:244-245`
...
- ✅ **CLOSED 2026-08-03 (`bd851f24`)** — the card asserted "It still matches your plan as it stands today." while the door beneath it showed the household never executed the advice; the holds line is now `recommendRecordHolds` = "It still lines up with the numbers you've entered." (`copy.ts:1519`, the defending comment `copy.ts:1489-1518`). *The live half of this entry is the strategy-naming ruling below.*
...
- ⚑ Anchor drift: `noChange: false` is `devSeeds.ts:1457` (not `:1322`); the winner literal is `:1443`
  (not `:1308`). `copy.ts:1483-1487` is the card-FAMILY header — the holds sentence's defending comment is
  `copy.ts:1489-1518` (added 2026-08-03), and it already carries the ⚠️ NEVER DEFEND THIS LINE WITH `noChange` warning.
...
- ⚠️ Anchor corrections: `healthOverlay.ts:296` is a **closing brace**, not a consumer (real seam `taxOverlay.ts:1689` + `:1731-1738` → `healthOverlay.ts:270`); the strings claiming the coupling is priced are `copy.ts:989-992`, not `copy.ts:933`.
...
- ⚑ **RETITLED 2026-08-02 — the filed claim ("cannot be seen") is HALF FALSE and the truth is worse.**
  It IS disclosed: `recommendationView.ts:88-91` → `copy.ts:2556-2557` → `RecommendationSurface.tsx:477-487`.
...
- ✅ **(XS) SHIPPED 2026-08-02 — the dead-end clause is GONE.** `copy.ts:2556-2557` now states the
  ...
  `copy.ts:2740-2741` under the comment at `:2730-2739` that states the both-ways coupling.
...
- **Near-term, honest, XS:** fix `copy.ts:1611-1612` only — it says a *withdrawal strategy* needs more pre-tax when only the **conversion** half does. Sequencing across taxable and Roth is real and rankable.
...
- **STILL OPEN — R23: the runner-up is retained but never IDENTIFIED**, and its `why` is one
  content-free constant (`copy.ts:1782-1783`) naming neither arm, while `payload.runnerUp.id`/`.policy` sit
...
- **🟡 Error copy speaks to the absurd reading:** birth year "62" → "Ages past 119 are beyond what
  ...
  `FIELD_OP_ALLOWLIST` `copyGuard.ts:267-270` already admits save/edit/add, so a sibling key needs no guard
