...
- **⚑ THE CHAIR'S FINDING, VERIFIED IN SOURCE AND NOT VISIBLE TO ANY READER: the loud primary is a NO-OP that does exactly what Cancel does.** `BudgetBuilder.tsx:267-282` — `apply()` with `items.length === 0` returns `onEscape()` when a budget governs, else `onClose()`; `budgetGoverns` is `budget !== undefined` (`budgetModel.ts:50-54`) and this household has none. So a full-width dark primary and a quiet text link are **functionally identical**, and nothing on the surface says so. That is insight 100's exact violation: the gesture promised an affordance, so it owes a rendered outcome. It renders none.
