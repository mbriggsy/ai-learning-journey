...
**COVERAGE HONESTY (the seed increment, filed):** the PRICED verdict affirmations (the NC/PA/FL residual sentences), the PRICED panel-row face, the CVD selected-picker face, and the `stalenessStateTax` gate note are NOT in this bundle — no dev seed carries a retirement state. They ride a **state-carrying seed increment** (seed + walk targets + fit arms + outcome pins) before their first cold read.
