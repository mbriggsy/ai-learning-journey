...
| [portfolio-holdings.md](portfolio-holdings.md) | How the portfolio is described to the engine — the as-built household-blend model (exact per-account % → one `stockWeight`), the shape-ratified multi-holding-entry design (opt-in; lands after U8 as an additive-within-v3 field shipped with its UI writer — no v3→v4 migration; the 2026-06-29 council refuted the old 'rides U8 or pay a migration' fork; build status lives in [the roadmap's You-Are-Here table](../roadmap.md#the-you-are-here-table)), and the basis / estate-character decisions (per-account-not-per-lot basis, the §1014 step-up). | §-less |
