...
- **KTD-8 — date-sweep invariance + the already-receiving anchor.** (a) Income is **Y-invariant** → it passes through `...overlayBase` **un-truncated** (do not add it to the date-sweep's stream truncation); `compileIncomeStreams` runs **once** per build, in `buildOverlay` inside the single `buildParams` pass (`src/intake/intakeMap.ts:566`), never per candidate. (b) `startAge ≤ currentAge` ⇒ **already-receiving, CLAMP to `t=0`, never reject** (start clamped to `max(0, startAge − currentAge)`). **The anchor:** for an already-receiving stream `annualRealToday` is the real value **at sim-year 0**, and the decay exponent is the **clamped `t`, never the stream's elapsed age** (else a plausible "honor the real age" impl over-deflates a real driver's pension). A golden pins `gross[0] === annualRealToday`.
...
Every deferral names its direction (the conservative-or-disclose rule). Read "disclosed" below as **disclosed here, in this record, with its direction named**: the only user-facing disclosure any of these deferrals ruled is the rental cliff magnitude, and it did not ship. (R40 has TWO shipped user-facing disclosures, each naming its own direction: KTD-9's steady-pay line, `workIncomeDisclosure` (`copy.ts:267-268`) — a different simplification — and, since 2026-09-06, the basis-recovery fine-tuning help, `incomeTaxableHelp` (`copy.ts:409-410`), which says the tool "holds that share steady for life, which reads a little better than reality once the paid-in amount would have been recovered" — the optimistic side, named correctly on the surface. **That help must never be reworded back toward "conservative"**: it shipped that way until 2026-09-06, and it named the direction backwards.) No shipped surface tells a user about the alimony payer-death or simple-COLA simplifications.
