...
Every deferral names its direction (the conservative-or-disclose rule). Read "disclosed" below as **disclosed here, in this record, with its direction named**: the only user-facing disclosure any of these deferrals ruled is the rental cliff magnitude, and it did not ship. (R40 has TWO shipped user-facing disclosures, each naming its own direction: KTD-9's steady-pay line, `workIncomeDisclosure` (`copy.ts:249-250`) — a different simplification — and, since 2026-09-06, the basis-recovery fine-tuning help, `incomeTaxableHelp` (`copy.ts:391-392`), which says the tool "holds that share steady for life, which reads a little better than reality once the paid-in amount would have been recovered" — the optimistic side, named correctly on the surface. **That help must never be reworded back toward "conservative"**: it shipped that way until 2026-09-06, and it named the direction backwards.) No shipped surface tells a user about the alimony payer-death or simple-COLA simplifications.
