...
Nothing was widened. The council wrote the law into `docs/architecture.md §12`: **the room is not the ink** — contain the ink, never widen the room to receive it, never sanction a bound no browser has rendered; where containment and in-plot seating are jointly unsatisfiable, the words LEAVE for flow reserved at their tallest. The remedy's SHAPE (min-content · wrap · flow row) is held for a cold read; the 320 readout arms are `test.fail`-declared (insight 120). An INSTRUMENT arm renders the held condition and REPORTS the quantities (`test.info().annotations` + a stdout line) without asserting them — the executable form of "render before sanctioning". *(2026-09-06, `d7bfcdb1`: his eye ruled the held shape and it was built the same day. The readout takes a MEASURED seat: in the plot while the widest column's box fits, otherwise a flow row reserved at its tallest (`readoutSeat` / `useReadoutSeat` / `ChartReadoutRow` in `src/viz/chartText.tsx`, architecture §12, insight 123). Every `HELD_READOUT_320` `test.fail` declaration was deleted with it, and the ladder instrument's became a dated ACCEPTED bound (`ACCEPTED_ONTRACK_OVERPRINT_PX` / `ACCEPTED_LABEL_GUTTER_PX`, `e2e/chart-text.spec.ts`). The instrument arms still REPORT their quantities, and now also assert the shipped seat and that bound.)*
