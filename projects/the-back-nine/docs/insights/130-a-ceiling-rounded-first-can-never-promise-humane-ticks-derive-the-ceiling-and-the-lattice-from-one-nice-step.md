...
When a cold read says a number "reads wrong", ask which STAGE authored it before touching the stage that printed it. A correct label on an ugly value is a *derivation* defect one step upstream; making the label more faithful only makes the ugliness legible. And compute the closed form BEFORE building: the whole rule was verified as a $1k-grid sweep from $100k to $12M plus every dev seed through the real engine (spine AND the date sweep) in a ten-minute probe — which is how the widest-tick fixture (`borderline`) was known to survive, the ceilings that would move (health 15M → 12.5M, borderline/budget 1.5M → 1.25M) were named in the brief, and the e2e regex/floor re-pins were measured rather than guessed.
