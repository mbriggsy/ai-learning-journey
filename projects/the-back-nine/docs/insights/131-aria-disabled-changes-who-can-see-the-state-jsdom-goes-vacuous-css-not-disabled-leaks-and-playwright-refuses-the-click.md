...
- Any `aria-disabled` primary whose press ANNOUNCES: the announcement must be the reason the surface RENDERS, never a status line for a run that is not pending — RothLever's incomplete face and HealthcareSheet's arrival face spoke "Working out both futures…" over a preview the effect had already withdrawn to idle until 2026-09-13 (the register entry *The two sibling sheets' blocked Apply*); a fourth instrument, the screen reader, was told a computation was in flight that was not, and the gates failed in two different ways: HealthcareSheet's live region was never pinned on that face at all, while RothLever's incomplete-plan arm PINNED `leverPreviewPending` as the EXPECTED sentence — a green oracle blessing the wrong words, flipped to the reason on 2026-09-13. A pinned live region is not coverage; pin the sentence the surface RENDERS.
