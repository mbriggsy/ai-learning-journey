...
- **SSA cohort survivor curves** confirmed against the real `table4c7.html` (bot-blocks automated fetch → manual fetch + committed snapshot) — load-bearing for the survivor differentiator and every Roth/solver survivor headline. **[CLEARED 2026-06-11 — with a NAMING CORRECTION: `table4c7.html` never existed ("4.C7" is TR table numbering). The real source = SSA HistEst cohort CSVs (TR2024/Alt2), committed sha256-pinned at `src/engine/reference/ssa-snapshot/`; `mortality.ts` re-derived 1:1 for the household's 1969/1972 cohorts (ratified): S(90|65) 0.3209/0.4348, couple 0.6162 — the retired Gompertz fit was OPTIMISTIC (0.554 couple), so the pin moves every survivor headline the honest direction.]**
- **Engine datasets pinned:** Bengen's Ibbotson intermediate-government series; a true long-term-corporate series for Trinity (cFIREsim-open is Shiller=government → directional, not exact). **[RE-SCOPED + CLEARED 2026-06-11 (ratified): `damodaranSeries.ts` is the named proxy pin — the Aaa-corporate Trinity arm lands 37/39 ≈ 94.9% (published 95.1%, same failing cohorts 1965/66), the 10-yr-Treasury Bengen arm is duration-conservative (SAFEMAX-analogue ~3.67% vs 4.15%, worst-cohort structure 1966/65/64 reproduced); bit-exact SBBI is RETIRED — commercial, edition-revised, and Bengen's own 1994–95 tail was estimated, so no canonical target exists. Survive/fail + independently-derived pinned window counts are the honest bar.]**
