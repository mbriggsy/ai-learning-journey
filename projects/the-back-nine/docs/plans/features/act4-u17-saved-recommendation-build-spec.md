...
- **WRITE:** the `RothLever` input, once labelled *"Starting how many years from now"* — wall-time words
  ...
  (`errRothStartPast`, `RothLever.tsx:280`, joined `SlottedErrorKey` at `copy.ts:1907`). The fresh default
...
each with its own heading and body (`copy.ts:1520-1529`). The recovery arm's name is deliberately **not**
...
GESTURE (`recommendSave*`) and the saved-record CARD (`recommendRecord*`), `copy.ts:1491-1616` — modelled on
...
- **The walk hard-flagged all six faces and NOTHING shipped from it** (`docs/caddie/cold-read-log.md`,
  ...
  *looking*, and its own deliverable was the plant that finally made them visible. Four of the six were
  ...
  record-card verb is closed too** — the showing-verb was replaced 2026-07-31 (`532cad82`; `copy.ts:1605`
  now reads "It may no longer fit the two of you.", with a standing prohibition on ever restoring it at
  `copy.ts:1576-1583`), and the false "still matches" holds line 2026-08-03 (`bd851f24`; `copy.ts:1568`).
