...
> (`taxOverlay.ts:922`) with genuine per-year consumption (074: a stamp nothing reads prices
...
- **S1 — consumption (taxOverlay).** The once-bound `partBBaseMonthly` (`taxOverlay.ts:1116-1125`)
  ...
  a REQUIRED param on both). `PART_B_PRICING_MODE` is `'trended'` (`taxOverlay.ts:922`), flipped in
...
- **S7 — copy + seeds + docs + gates.** The seed re-tunes ran under record-before-retune. THE DRIFT
  ...
  the extra-coverage premiums ALONE (the shipped string in `src/ui/copy.ts:1166`), with the roth
