...
- `nc` / `pa` / `fl` / `elsewhere` = clones of `retiredOnTrack` (66/65 both retired — the state clause renders ONLY inside the `medicarePricedNote` block, `ConfidenceStatement.tsx:412-425`, so the household must be all-65+) with `retirementState: 'NC' | 'PA' | 'FL' | 'elsewhere'` respectively.
- `datenc` = clone of `stillWorkingAllMedicare` (the `date65` seed) + `retirementState: 'NC'` — the date route's affirmation mount (`FuckOffDate.tsx:338-346`) rides its OWN producer (`dateStatePriced`, `intakeMap.ts:718`); insight 080's lesson: the second producer gets its own live witness.
...
- New `AGED_PLANTS` entry (`devSeeds.ts:880-883`). Extend `doctorStaleVault` (`:839-869`) **conditionally — only when the scenario carries a state-tax vintage stamp** — to write a divergent, plausible OLDER NC profile (the `taxVintageDetail` "pre-OBBBA dev fixture" idiom at `:857`), coherent with the −2y / −760d anchor. `stale`/`datestale` outputs stay byte-identical (their bases carry no state) — pin that.
- The clock's firing predicate is `staleness.ts:208-214` (saved stamp present ∧ household state priced ∧ `stateProfileKey` diverges — **read the real stamp shape at `stateTax.ts:383` / `:405` before writing the doctor**). FL never fires (constitutional $0) — that's why the base is `nc`.
- Must hydrate with a **CLEAN badge** (the drifted-vault clean-badge law) AND fire `controls.stateTaxMoved` → the `stalenessStateTax` note (`copy.ts:1022`) at the gate.
...
- Every new seed: `pricedStateForRun(draft)` === its state (or `undefined` for `elsewhere`) via the REAL producer (`intakeMap.ts:728`) — never draft truthiness.
- `nc`: an engine INEQUALITY pin vs the state-absent twin (the `'order'` seed idiom, `devSeeds.test.ts:287`) — NC tax must move a real wire statistic. Record the engine-proven outcomeState; on drift, re-tune the account knob, never loosen the pin (the standing C3 law — record un-retuned drift first if any existing pin moves, expected NONE since no existing seed is touched).
...
- `datenc`: async date-route pin (provisional tier, 120s-timeout idiom, `devSeeds.test.ts:156-251`) + the `dateStatePriced` output === `'NC'`.
