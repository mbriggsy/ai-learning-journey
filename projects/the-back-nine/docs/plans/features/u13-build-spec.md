...
1. **`savedAt?: number`** (model.ts:1772) — finite integer **epoch-day**, encrypted in the model. The unit is
...
2. **`taxVintageDetail?: TaxVintageV3`** — atomic object minted by **`taxVintageStamp()`** (tax.ts:394),
   ...
   - Neither the RMD-age rule nor the senior-bonus sunset is stamped, and **neither got a clock** — the
     ...
     (`rmdStartAge`, tax.ts:161 — the figures live there, never re-typed here) is birth-year-cohort-keyed
...
1. **The senior-bonus sunset clock was REMOVED, and the refuted premise is why.** The ratified plan called
   ...
   **CLOSED the same day** — council-ratified 8/10 (wf_a5ccd576-048, no veto; the honesty hawk's opening
   ...
   insight-019 handling). `seniorBonusFor` (taxCore.ts:83) now prices the bonus ONLY in calendar years
