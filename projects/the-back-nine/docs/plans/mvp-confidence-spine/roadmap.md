...
| The tax-aware "without" arm drifts from the spine headline the user anchored on → distrust | Reframe-at-lever-open ("now tax-aware, both arms"); the OFF-by-default reduce-to-spine invariant (all four conditions — Phase-3 contract #1); saved spine headline never silently overwritten (Phase-3 Unit 8) |
| Death-order treated as a re-draw/re-index instead of a conditional filter → CRN breaks and the survivor delta jitters | Fixed per-spouse longevity lanes in the max-horizon matrix; the death-order conditional **filters the existing path population** (same seed/draws — no re-draw, no re-index); added to contract #1's CRN-safe enumeration + a matching CRN test (Phase-3 contract #3) |
