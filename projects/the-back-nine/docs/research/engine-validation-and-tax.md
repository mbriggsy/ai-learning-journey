...
The product **recommends** a strategy (withdrawal sequencing + Roth conversion), so the contract GROWS beyond "the number is right." The cases below validate the engine's *number*; a recommender needs three more classes, all **gating before the solver is allowed to speak** (a wrong recommended strategy costs real dollars). The oracle and the solver shipped in Act 4 (the U14 validation harness cb5aa00b and the U15 solver core 6ca61380, both 2026-07-18 — `src/engine/validation/`, `src/engine/solver/`); the contract they implement is recorded here:
...
|---|---|---|---|
| 50/50 | 4% infl-adj | 30 yr | **95%** (the published 39/41 ≈ 95.1%; the engine's committed Damodaran Aaa-corporate arm, 1928–1995, asserts the derived **37/39 ≈ 94.9%** with the same failing cohorts {1965, 1966} — a pinned window count, never published-figure equality, which the `historical.ts` header retires) |
...
- **The committed proxy arms (P1-exit pin pass, 2026-06-11; series `src/engine/reference/damodaranSeries.ts`, counts pinned in `src/engine/__tests__/historical.test.ts`).** Bit-exact Trinity/Bengen equality is **retired**: SBBI is commercial and edition-revised, and Bengen's 1994–95 tail was estimated. So validation is survive/fail, with window counts pinned to an independent derivation (DND/012). Damodaran coverage starts in 1928, giving 39 rolling 30-yr windows against Trinity's 41. **Trinity corporate arm** (Moody's Aaa constant-maturity total return, 50/50, 4%, 30 yr): **37/39 ≈ 94.9%** vs the published 39/41 ≈ 95.1%. The failing cohorts are exactly {1965, 1966}, the same ones Trinity shows. **Bengen arm** (10-yr Treasury, longer duration than his 5-yr intermediate government, so it runs **conservative**): at 4.0% it scores 36/39, and the failing cohorts are exactly {1964, 1965, 1966}. His published 4.15% scores 35/39 here. The **SAFEMAX-analogue is ~3.67%**, bracketed by the test (all 39 windows survive at 3.60%, not all at 3.75%), against his published 4.15%. The proxy never reports a higher safe rate than the 5-yr truth.

### Monte Carlo calibration (a band, NOT an equality)

- A pure i.i.d. Monte Carlo scores a given withdrawal rate **more pessimistically** than a historical backtest (i.i.d. draws ignore mean-reversion and can string arbitrarily bad runs). 4%/30yr lands **high-80s to ~90%**, *below* the 95% historical. **This is by design** — assert only a *range* for MC, use the historical backtest's pinned window counts (survive/fail per cohort) as the oracle — never bit-exact equality with a published figure (`historical.ts` header).
...
> **The falsifiable IN/OUT line:** a tax/health effect is **IN iff withdrawal sequencing or a conversion can move it.** IN: ordinary brackets, standard deduction, RMDs, SS-taxation, MFJ→single, **ACA-PTC (pre-65), IRMAA (post-65), cap-gains / qualified-dividend stacking**. Also IN: **state income tax for the priced roster {NC, PA, FL}** (the state-tax unit, c7e5936c 2026-07-15; product.md D6 amended 2026-07-18 — NC taxes withdrawals AND Roth conversions at its flat rate, so the conversion lever moves it; the scope sentinel is `stateIncomeTax` in `src/engine/constants/tax.ts`, the figures `src/engine/constants/stateTax.ts`, re-verified by `pnpm verify:state-tax`, spec [state-tax-build-spec.md](../plans/features/state-tax-build-spec.md)) and the lever-sensitive **§1014/IRD heir-tax adjustment** (the leave-more objective). OUT-but-disclosed: NIIT, and state income tax for every state outside the priced roster. **Two distinct MAGI calculators** (ACA-MAGI ≠ IRMAA-MAGI). **Legislative landmine:** the enhanced ACA subsidies **expired 12/31/2025, unre-enacted as of 2026-06-04** → model the **400% FPL cliff as the 2026 base case**, expose "enhanced" as a scenario toggle, **re-verify every build** (CI-gated by `pnpm verify:aca`).
...
**The One Big Beautiful Bill Act (OBBBA), signed 2025-07-04, made the TCJA individual rate structure (10/12/22/24/32/35/37) and the elevated standard deduction PERMANENT** — so 2026 is **not** a TCJA-sunset reversion to pre-2018 brackets, which is what would have happened absent legislation (TCJA individual provisions were set to expire 12/31/2025). **Why this matters for the build:** the "tax-table vintage" staleness clock is therefore *not* tracking a known one-time 2026 reversion — it tracks (a) annual inflation indexing of a permanent bracket structure (a `taxYear` bump) and (b) any future law change, a real extension or repeal of the senior-bonus deduction included (a `legalBasis` bump) — the two fields of `taxVintageStamp()` in `src/engine/constants/tax.ts`. The senior-bonus sunset and the RMD-age step (both below) are deliberately NOT staleness clocks. The engine prices both deterministically (calendar-year and birth-year keyed), so a re-open after either date recomputes byte-identical (`src/store/staleness.ts` header). **Stamp the legal basis (OBBBA-2025), not just the year**, so a future statutory change is falsifiable rather than mistaken for inflation drift.
...
- born **≤ 1950 → 72**; born **1951–1959 → 73**; born **1960 or later → 75** (the 75 step is **effective 2033**). (The 1959 statutory drafting glitch resolves to **73** — current standard interpretation.) RMD = the IRS **Uniform Lifetime Table** divisor (the **Joint Life & Last Survivor Table** when the sole-beneficiary spouse is more than 10 years younger) applied to the prior-year-end pre-tax balance; first RMD due by April 1 of the year after reaching RMD age. **The age is a per-person function of birth year and is legislatively scheduled to change (the 2033 step) — so it is a vintage-stamped / birth-year-keyed input, never a hardcoded literal.**

### Temporary Senior Bonus Deduction (OBBBA) — a sunsetting provision the engine prices by calendar year

- **$6,000 per person age 65+ ($12,000 MFJ)**, on top of the standard deduction, claimable whether itemizing or not. **MAGI phase-out:** begins single > $75k / MFJ > $150k, reduced 6% per dollar over. Deduction = **(# spouses 65+) × max(0, $6,000 − 6% of (MAGI − $150k))** on a joint return — the reduction applies to EACH qualifying spouse's $6,000 (IRC §151(d)(5)(C)(iii) reduces "the $6,000 amount"; IRS Schedule 1-A Part V lines 32–37 compute line 35 once and enter it for each spouse on 36a/36b). Fully gone: **single > $175k**; **MFJ > $250k whether one or both spouses are 65+**. ⚑ **CORRECTED 2026-09-25:** this strand originally read "MFJ > $350k when BOTH are 65+ ($12k)" — one pooled $12,000 reduced once — and the engine priced that reading from the M3 build until 2026-09-25, over-granting up to $6,000 of deduction to both-65+ couples in the $150k–$350k band (rosy). Schedule 1-A refutes it; the register's closed *The OBBBA senior bonus phased out of ONE pooled $12,000…* carries the record. **Not inflation-indexed** (no COLA clause in P.L. 119-21; §151(d)(4) excludes paragraph (5); Rev. Proc. 2025-32 lists no §151 figure) — the engine deflates the $6,000 and both starts per sim year since 2026-09-25. *(Sources: P.L. 119-21 §70103 enrolled text; IRS Schedule 1-A (Form 1040); IRS FS-2025-03.)* **Available tax years 2025–2028 only; SUNSETS after 2028** unless extended. → The engine PRICES the window: `taxCore.seniorBonusFor` credits the bonus only in calendar years [2025..2028] (`seniorBonus.effectiveFrom`/`sunsetAfter`, shipped 2026-07-09). A calm note at the sunset crossing was REFUTED by the U13 ultramode review — a correctly-priced save recomputes byte-identical after 2028 (`src/store/staleness.ts` header; [architecture.md §8](../architecture.md#8-constants-discipline-srcengineconstants)).

### Engine tax + health scope (decided 2026-06-04 — "the most complete picture")

**IN the two-control solver's tax + health model (withdrawal sequencing + Roth conversion):** federal MFJ + single ordinary-income brackets, the standard deduction (+ age-65 additions + the senior-bonus deduction with its phase-out & sunset), birth-year-derived RMDs (Uniform Lifetime; Joint Life & Last Survivor for a >10-yr-younger sole-beneficiary spouse), each year's withdrawal/conversion taxed as ordinary income stacked on that year's other ordinary income, **Social-Security provisional-income taxation (the "tax torpedo")**, **capital-gains / qualified-dividend stacking from taxable-account withdrawals**, and **income-dependent healthcare — pre-65 ACA-PTC (400% FPL cliff is the 2026 base case; "enhanced" = a scenario toggle) and post-65 IRMAA (2-year MAGI lookback, a distinct MAGI definition, hard cliffs)**, and the lever-sensitive **§1014/IRD heir-tax adjustment** (the leave-more objective) — every effect that withdrawal sequencing or a conversion can move (see [pre65-healthcare.md](pre65-healthcare.md)). **Also IN since the state-tax unit (2026-07-15; D6 amended 2026-07-18):** state income tax for the priced roster {NC, PA, FL} — a second addend inside the same gross-up fixed point, and NC taxes conversions, so the conversion lever moves it ([architecture.md §7.1](../architecture.md#71-tax-and-accounts-overlay-taxoverlayts)). **OUT-but-disclosed (next to the delta, candidate future levers):** NIIT (3.8% surtax — neither control moves it) and state income tax for any state outside the priced roster (context the tool does not price).

**The falsifiable IN/OUT line (so the scope stays principled, not self-justifying):** a tax/health effect is **IN** iff **withdrawal sequencing or a Roth conversion can move it** — ACA-PTC and IRMAA are income-dependent, so a sequencing/conversion change shifts ACA-MAGI or IRMAA-MAGI across their cliffs, and an omitted cliff *inverts which strategy wins*, not just its size (this is exactly why they come IN). It is **OUT-but-disclosed** iff neither control can move it — NIIT (a 3.8% surtax) is the one lever-inert OUT member. State income tax outside the priced roster {NC, PA, FL} is OUT-but-disclosed for a different reason: coverage, not lever-inertness (NC's conversion DOES move NC's bill, which is exactly why the roster is IN). The tool does not price it, and coverage grows with the roster, never a 50-state brush ([product.md](../product.md) D6, amended 2026-07-18).
...
- **These thresholds are NOT inflation-indexed — frozen since 1983 ($32k) / 1993 ($44k).** So they have **no vintage/staleness clock** (unlike the brackets, which inflation-index annually): they are constants, not a dated fixture, and a frozen constant cannot go "stale." (This is *why* more retirees are caught each year — a feature to model honestly, not a bug.) **MODELED since 2026-09-24, not before:** the engine runs in REAL dollars, so the statute's NOMINAL lines must be DEFLATED per sim year — `taxableSocialSecurity(…, calendarYear)` divides them by `cumulativePriceIndex(calendarYear)` (`src/engine/priceIndex.ts`, the one CPI path the Part B schedule already rides). From 2026-06-06 (M4's ship) to 2026-09-24 the engine held the real line flat, i.e. it INDEXED what Congress froze — the rosy direction this very strand warned against; measured at the fix, the shipped `retired` seed moved from 9/10 on-track to 8/10 borderline (the register entry *The Social Security taxation thresholds are FROZEN by law…* carries the full before/after table).
...
**Constants-module shape:** each schedule is stored as a per-month rate in fractions of 1%, as `{numerator, denominator}` (`workerReduction` 5/9 then 5/12; `spouseReduction` 25/36 then 5/12; `delayedRetirementCredit.ratePerMonth` 2/3). The sub-engine (`perMonthReciprocal` in `socialSecurityBenefit.ts`) derives the integer reciprocals 180 / 240 / 144 for the reductions, so 62/FRA67 falls out as exactly `168/240 = 0.7000`, and it applies the DRC as the exact rational `(300 + 2·months)/300` (2/3 of 1% = 1/150 per month). The DRC month cap is derived as `delayedRetirementCredit.throughAge × 12 − fraMonths` (36 @FRA67, 46 @FRA66y2m — never a literal 36). `survivorReduction` stores `maxReductionPct: 0.285` from `earliestAge: 60` (the per-month fraction is computed from the 60→survivor-FRA span), plus `disabledEarliestAge: 50` for the DWB flat-28.5% floor (stored; the sub-engine defers DWB). `ribLim.floorPctOfDeathPia = 0.825`; `spousalRate = 0.5`; `deemedFilingDobCutoff = { year: 1954, month: 1, day: 2 }`.
...
- **SECURE 2.0 §109** — the 60–63 "super catch-up" (the greater of $10,000 or 150% of the **2024** age-50 catch-up — $7,500 → $11,250 — with that greater-of amount COLA-indexed from 2026 in $500 round-down steps, IRC §414(v)(2)(E)(i) / (C)(i); so 2026 is **$11,250**, NOT 150% × the 2026 $8,000 = $12,000 — insight 022, `employerPlan2026`; an optional plan feature) — carried as `legalBasis` provenance.
...
| RMD divisors (Uniform Lifetime Table + Joint Life & Last Survivor Table) | **RESOLVED / PINNED** | ULT pinned against IRS Pub. 590-B Table III; JLLS transcribed from eCFR 26 CFR 1.401(a)(9)-9(d) and cross-verified against Pub. 590-B Table II (insight 009); both in the canonical constants table (`src/engine/constants/tax.ts`) |
...
| Bengen 1966 bit-exactness (depends on the exact Ibbotson intermediate-government dataset) | **DIRECTIONAL by design** | Treated as a directional survive/fail test, not a golden number — the dataset gate is recorded above; do not assert a bit-exact SAFEMAX without the Ibbotson series. The committed proxy is the Damodaran 10-yr-Treasury arm (`damodaranSeries.ts`, counts pinned in `historical.test.ts`, see *Deterministic golden*) |
...
The storage + crypto layer is **local-first and zero-knowledge**: the financial picture is entered by hand, encrypted at rest in the browser, and never leaves the device. The load-bearing invariants — AES-GCM-256 under a PBKDF2-600k-hardened key, the non-extractable `CryptoKey`, the recovery-passphrase (a second user-chosen passphrase, the U8 rework) + mandatory encrypted-export survivor backstop, the strict CSP — live once in [architecture.md §7.3 / §10](../architecture.md). This section keeps the *comparative* decisions behind them: why each rejected alternative was rejected.
...
- **Login recovery is separate from data-decryption recovery.** The recovery passphrase recovers the **data** (it derives the key that unwraps the vault's data key); an email/SMS-style reset is **login-only** and would strand the encrypted data. So the survivor's door is the recovery passphrase + the mandatory encrypted export at first save, and IndexedDB eviction (WebKit ITP's ~7-day clear) is treated as best-effort — the encrypted export file, opened with the recovery passphrase, is the real durability backstop.
