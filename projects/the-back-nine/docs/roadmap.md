...
| **U7** | Confidence statement surface + outcome-state system + survivor readout; copyGuard born here | in-progress | Path B. The SURFACE is built, **N=1 cold-read-cleared (2026-06-27)**, and **`/ultramode-code-review`-cleared** (8 lenses, 0 P0/P1; [insight 044](insights/044-a-can-never-happen-comment-is-a-claim-about-the-gate-not-a-fact.md)): `ConfidenceStatement` (verdict-first, wires the band on demand), the `resolveBandData` producer + its fail-loud seam, the non-color verdict signals, `outcomeStates`, the `copy` verdict-grammar, `copyGuard`, the `?preview` harness. Ahead (keeps it in-progress): the survivor **engine output** + `SurvivorReadout` (ENGINE-BLOCKED — no survivor-conditioned distribution emits yet), and the live-app wiring (D2 — carries the $0-portfolio band-suppression + insight-035 obligations) |
...
| **Tests** | 1166 vitest across 57 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
