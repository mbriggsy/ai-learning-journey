...
| **Tests** | 1959 vitest across 111 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
...
**Act 3 (Controls, U9–U13) is the live act** — U9 + U10 + U11 are built ([docs/plans/3-controls.md](plans/3-controls.md)); **U12 (sharpen loop) is next to build**. The hard pre-Act-3 gate's engine + render halves are satisfied (`?seed=dip` — the budget-collision mechanism; the council-log gate block carries the record); Briggsy's cold-read batch (the dip verdict + U11's cliff/shadow-rate tone) is the open half and gates U11's SHIP. Act 4 (Recommendation) — the actual differentiator — follows Act 3.
