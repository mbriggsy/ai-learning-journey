...
| **U7** | Confidence statement surface + outcome-state system + survivor readout; copyGuard born here | in-progress | Path B. The SURFACE is built, **N=1 cold-read-cleared (2026-06-27)**, and **`/ultramode-code-review`-cleared** (8 lenses, 0 P0/P1; [insight 044](insights/044-a-can-never-happen-comment-is-a-claim-about-the-gate-not-a-fact.md)): `ConfidenceStatement` (verdict-first, wires the band on demand), the `resolveBandData` producer + its fail-loud seam, the non-color verdict signals, `outcomeStates`, the `copy` verdict-grammar, `copyGuard`, the `?preview` harness. Ahead (keeps it in-progress): the survivor-conditioned engine distribution (e1) is **built** (2026-06-27 — `simulate`'s opt-in `survivorConditioned` surface: the equal-weight survivor-conditioned survival fraction, externally-derived + reduce-to-spine + byte-identity tested); remaining = the income step-down magnitude, the `confidence.ts` survivor reading, and `SurvivorReadout` (e2, eye-oracle — deferred to a cold-read), plus the live-app wiring (D2 — carries the $0-portfolio band-suppression + insight-035 obligations) |
...
| **Tests** | 1177 vitest across 58 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
