...
| **Tests** | 2108 vitest across 118 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
...
**Act 3 (Controls, U9–U13) is the live act** — U9 + U10 + U11 are shipped and **U12 (sharpen loop) is BUILT + live-verified** ([docs/plans/3-controls.md](plans/3-controls.md)); U12's ultramode review is folded (2026-07-09); the owed closer is Briggsy's cold-read batch (the bad-news truer-picture transition is a formal Act-3 exit condition), then **U13 (re-entry + staleness) is next to build**. The 2026-07-08 gate batch cleared (the dip verdict + U11's six stations). Act 4 (Recommendation) — the actual differentiator — follows Act 3.
