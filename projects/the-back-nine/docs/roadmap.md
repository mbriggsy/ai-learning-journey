...
| **Tests** | 1851 vitest across 105 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
...
**Act 3 (Controls, U9–U13) is the live act** — U9 + U10 are built ([docs/plans/3-controls.md](plans/3-controls.md)); **U11 (healthcare surfaces) is next**. The hard pre-Act-3 gate's engine + render halves are satisfied (`?seed=dip` — the budget-collision mechanism; the council-log gate block carries the record); Briggsy's dip cold-read is the open half. Act 4 (Recommendation) — the actual differentiator — follows Act 3.
