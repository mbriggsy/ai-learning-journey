...
| **U7** | Confidence statement surface + outcome-state system + survivor readout; copyGuard born here | in-progress | Path B. The SURFACE is built, **N=1 cold-read-cleared (2026-06-27)**, and **`/ultramode-code-review`-cleared** (8 lenses, 0 P0/P1; [insight 044](insights/044-a-can-never-happen-comment-is-a-claim-about-the-gate-not-a-fact.md)): `ConfidenceStatement` (verdict-first, wires the band on demand), the `resolveBandData` producer + its fail-loud seam, the non-color verdict signals, `outcomeStates`, the `copy` verdict-grammar, `copyGuard`, the `?preview` harness. Ahead (keeps it in-progress): the survivor-conditioned engine surface — e1 (survival fraction) + e1b (the income step-down magnitude) + e1c (the `confidence.ts` survivor reading) — is **built + adversarially verified** (2026-06-27; a 4-lens adversarial pass caught + fixed a P1 understatement where an fd-anchored counterfactual went negative for pre-claim deaths, the §202 widow benefit front-running an unclaimed retirement benefit — fixed with a steady-state anchor, regression-guarded); remaining = `SurvivorReadout` (e2, eye-oracle — deferred to a cold-read), plus the live-app wiring (D2 — carries the $0-portfolio band-suppression + insight-035 obligations) |
| **U8** | First-Save flow + recovery-phrase display + mandatory export + passphrase-strength gate (`zxcvbn-ts` ≥ 3 ∧ length ≥ 12) | in-progress | The `ScenarioV3` codec arm + `AnyScenario` membership are **built + adversarially verified** (2026-06-27 — `checkV3Fields` + the separate `checkPersonV3`/account/health/income/ticker validators; a 4-lens pass caught + fixed a P1 cardinal-sin false-accept where the income entity fractions weren't [0,1]-gated, the codec being the sole such gate on restore). Remaining = the first-Save **ceremony** (eye-oracle, deferred to a cold-read) + load-on-return wiring. The portfolio-holdings multi-holding field stays an OPEN ATC call (`docs/decisions/portfolio-holdings.md` — unratified) |
...
| **Tests** | 1212 vitest across 58 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
