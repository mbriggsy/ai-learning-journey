...
| **U11** | Healthcare surfaces (ACA cliff + enhanced toggle + re-verify gate; IRMAA cliffs; HSA entry) over the U3 overlay | shipped | Built 2026-07-03 (council-ratified design → the magiLandscape rail/readout module, engine-derived cliff-aware bracket-fill ceilings + the picker join, the per-year healthReadout wire emission, `enhancedSubsidies?`+`healthcareVintage` additive-in-v3, the Healthcare door/sheet + shadow-rate readout + regime what-if lever, the post-65 unpriced-Medicare disclosures; ultramode-reviewed, live-verified, `?seed=health`). **The ship-gating cold-read batch CLEARED 2026-07-08** (all six stations walked; its two findings — the laptop one-frame regression and the bare comparison chart — were fixed, live-verified, and pushed the same day: 6f8305de + fcc35556). The council-REQUIRED post-65 Medicare pricing engine unit stays filed (TODO) — the disclosure is the honesty surface until it ships. |
...
| **Tests** | 2159 vitest across 119 files (Vitest 4, `globals:false`); externally-derived goldens, absence-tests paired with presence companions | `pnpm test` |
