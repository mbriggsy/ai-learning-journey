...
describe('canonical constants — shape & provenance (contract #6)', () => {
  ...
  it('the three named research gaps are unsourced sentinels (not plausible defaults)', () => {
    expect(isUnsourced(taxConstants.ordinaryBracketsSingle)).toBe(true)
    expect(isUnsourced(taxConstants.uniformLifetimeTableDivisors)).toBe(true)
    expect(isUnsourced(taxConstants.capitalGainsBreakpoints)).toBe(true)
    // plus the single-filer age-65 addition the research also omitted
    expect(isUnsourced(taxConstants.age65AdditionSingle)).toBe(true)
  })
  ...
  describe('single-source: no overlay number is inlined outside the constants module', () => {
    const DISTINCTIVE = ['768700', '512450', '218000', '84600', '202.9', '689.9']
