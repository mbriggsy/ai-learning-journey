 ...
 *
 * THE THREE-WAY ENTRY (U8 decrypt-on-return). On mount (when there is no dev `?seed`) `probeVault()`
 * reads IndexedDB: `vault` → the Unlock screen; `damaged` → the interim damaged notice (restore-from-
 * file is surface 4); `no-vault` (fresh or evicted) → ColdStart. A brief neutral hold covers the
 * probe so a returning user never flashes "brand new?" and a fresh user never flashes "welcome back".
 * The probe is a DYNAMIC import so `vaultSession`'s crypto/KDF/backup graph stays out of the entry
 * chunk (it warms off the first-paint path here, ready by the time Unlock's submit needs it).
 *
...
/** `seed` is the DEV-only `?seed=<key>` value (always null in prod — the gate lives in main.tsx). When
 *  present we skip the probe and mount the intake straight away so IntakeApp applies the seed. */
export function App({ seed }: { seed?: string | null }) {
  const [entry, setEntry] = useState<Entry>(seed != null ? { kind: 'began', hydrate: false } : { kind: 'probing' })

  useEffect(() => {
    void import('./IntakeApp') // warm the chunk behind the cold-start / probe frame
  }, [])

  // The startup vault probe (skipped when a dev seed drives the mount). Dynamic import keeps the
  ...
  // unlock/result <h2> headings nest under it (ColdStart owns its own visible <h1>; the probe hold is
  // pre-content). One stable app-title h1 across all these SPA states — never blinked (council
  // 2026-06-29).
  ...
  return (
    <>
      ...
      {entry.kind === 'probing' && null /* brief neutral hold — never the wrong screen */}
      {entry.kind === 'cold' && <ColdStart onBegin={() => setEntry({ kind: 'began', hydrate: false })} />}
      {entry.kind === 'unlock' && <UnlockScreen onUnlocked={() => setEntry({ kind: 'began', hydrate: true })} />}
