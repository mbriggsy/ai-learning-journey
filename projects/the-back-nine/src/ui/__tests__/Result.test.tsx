...
describe('Result — the save slot states', () => {
  ...
  it("'saving' renders the calm pending status — no buttons, never an alert", () => {
    renderSave({ kind: 'saving' })
    expect(screen.getByText(copy.resavePending)).toHaveAttribute('role', 'status')
    expect(screen.queryByRole('alert')).toBeNull()
    expect(screen.queryByRole('button', { name: copy.resaveCta })).toBeNull()
  })

  it("'failed' renders the refusal as an ALERT with the mapped copy and a live retry", () => {
    const onRetry = vi.fn()
    renderSave({ kind: 'failed', errorKey: 'saveErrorReadOnly', onRetry })
    const alert = screen.getByRole('alert')
    expect(alert.textContent).toBe(copy.saveErrorReadOnly)
