 ...
 * The widest shipping tick measures 38.5 CSS px of Windows ink at --text-xs — the six-glyph "$0.25M" /
 ...
 * against ("$0.375M" / "$1.125M", 45 px Windows — quarters of a 1.5-rung ceiling): niceLattice picks the
 ...
 *  tripwire. 38.5 is the six-glyph "$0.25M" / "$0.75M" / "$1.25M" on Windows (36.0 Linux); it was 45 (42) for the seven-glyph
  ...
  it('the worst-case (breakpoint) band-figure renders a tick column wide enough for the widest catalog dollar', () => {
    ...
    expect(
      ...
      `worst-case band-figure ${Math.round(bandFigure)}px renders a ${tickColumnPx.toFixed(1)}px tick column — ` +
        `narrower than the widest catalog dollar (${WIDEST_TICK_INK_PX}px of Windows ink at --text-xs). ` +
    ...
    // units (42.2px). Card 10's honest 6.5px Windows narrowing of that dollar (6.0 Linux) shrank that reach:
    ...
    // the cross-platform ink cushion (a 2.5 px spread — 38.5 Windows vs 36.0 Linux, both measured;
    // insight 118), not a knob for choosing which breakpoints red.
