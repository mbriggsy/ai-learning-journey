 ...
 *  46.2 px ink column — the 50.6 px gutter less 4.4 px — so 38.5 CSS px of WINDOWS ink, re-measured
 *  in the browser gate 2026-09-12 for Card 10, derives ~7.7 px of slack; the gate MEASURES it 7.6 px
 *  inside the figure at PHONE (10.2 px on Linux). Linux CI MEASURES that dollar at 36.0 px (run 34707283988,
 *  the first Linux verify:fit on Card 10, all six arms) — 2.5 px narrower than Windows, ~0.42 px
 *  per glyph (insight 118; FreeType rounds advances to whole pixels, DirectWrite keeps them
 *  fractional); the pre-run 38.5 − 6 × 0.43 ≈ 35.9 derivation from the seven-glyph pair was 0.1 px LOW.
 *  Until Card 10 the widest was the SEVEN-glyph "$0.375M" / "$1.125M" quarter of a $1.5M ceiling
 *  at 45 px Windows / 42 px Linux (the insight-118 pair), which the nice-step lattice can no longer
 ...
 *  (chart-text.spec.ts ARMS), the figure is 238px — a 35.7 px column — and that dollar borrows the
 *  drawer's padding instead: 2.9px Windows / 0.3px Linux, both measured, bounded live by assertTickColumn.
