# TOP-DOWN RACER v03
## GSD Interview Prep & Architecture Spec
*Prepared by Briggsy — March 2026*
*The pre-baked answers doc. War-game everything before GSD asks.*
...
**Decision: Replace Web Audio API synthesis with layered samples for engine sound.
Keep synthesized SFX for everything else.**

**Rationale:**
- Engine sound is the single biggest audio quality gap in v02. Synthesis produces a
  convincing but clearly synthetic engine note. Layered samples sound commercial.
- Everything else (skid, collision, checkpoint chime) works fine synthesized.
- Use Ludo.ai's audio generation to create 3–4 engine loop samples at different RPM
  bands. Blend between them based on `carState.speed`. Standard racing game technique.

**Audio architecture:**
```
Engine sound: 3 WAV loops (idle, mid, high RPM) → GainNode crossfade based on speed
SFX: Web Audio API synthesis (keep from v02 — it's good)
Music: Optional — Ludo.ai generated ambient racing music for menu
```
...
### Inherited from v02 (no changes)
- **GSD** — orchestration framework, phased roadmap, atomic commits
...
- **Compound Engineering** — `/deepen-plan` pre-execution bug catch
...
**v03 proves two things simultaneously:**
...
aesthetically with commercially produced games.

Second — AI opponents built with this approach can **generalize, not memorize**.
A v03 AI that drives circuits it never trained on isn't just a party trick. It's
...
- v03 new project: `C:\Users\brigg\ai-learning-journey\projects\top-down-racer-03`
- Same stack, same tooling, new target directory
- v03 starts from a fresh scaffold — do NOT copy v02's src/ wholesale
