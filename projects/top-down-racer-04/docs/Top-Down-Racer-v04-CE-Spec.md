...
production-quality visual assets using the Gemini Imagen 3 API (fully autonomous —
...
Asset generation is fully autonomous via Gemini Imagen 3 API — no human art tooling required.
...
### ADR-02: Asset Pipeline — Autonomous Gemini Imagen 3 API

**Decision: Fully autonomous asset generation via Gemini Imagen 3 API. No human art tooling.**
...
Imagen 3 API directly from a build script. Same underlying model, no middleman.

**Pipeline Design:**

```
GENERATION PHASE (fully autonomous — Claude Code runs this)
  └─ scripts/generate-assets.ts  → calls Gemini Imagen 3 API → /assets/raw/
...
**Asset generation approach (all Gemini Imagen 3 API):**
...
|-------|-------|-------------|
| 0 | Asset Generation | Run `generate-assets.ts` — all sprites, textures, backgrounds generated via Gemini Imagen 3 API and dropped into `/assets/raw/` |
...
- Script calls Gemini Imagen 3 API with typed prompts for each asset
...
**Asset inventory (generated by Gemini Imagen 3):**
...
- **Gemini Imagen 3 API** — autonomous asset generation (`GEMINI_API_KEY` in `.env`)

### New for v04
- **Asset Generator** (`scripts/generate-assets.ts`) — calls Gemini Imagen 3, drops PNGs into `/assets/raw/`
...
A: No. Asset generation uses the Gemini Imagen 3 API directly via `scripts/generate-assets.ts`.
