...
export function mount(container: HTMLElement, state: AppState): void {
  ...
  setTopBarInstruction('\u{1F3A9} Choose the next Mayor');
  ...
  list.dataset.testId = 'special-nominate-picker';

  const confirmBtn = document.createElement('button');
  confirmBtn.className = 'action-btn';
  confirmBtn.dataset.testId = 'special-nominate-confirm';
  confirmBtn.textContent = '\u{1F3A9} Confirm';
  ...
  for (const p of players) {
    ...
    li.dataset.testId = 'special-nominate-player';
    if (!p.isAlive) {
  ...
  confirmBtn.addEventListener('click', () => {
    ...
    confirmBtn.disabled = true;
    sendAction({ type: 'special-nominate', targetId: selectedId });
  });
