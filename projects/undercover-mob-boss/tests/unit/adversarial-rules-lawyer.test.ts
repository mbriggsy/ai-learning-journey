...
describe('Adversarial: Mob Boss election check timing', () => {
  it('Boss elected as commissioner at 2 bad policies, then 3rd bad enacted in same round — game continues', () => {
    // Setup: 2 bad policies enacted, mob boss is nominated as commissioner
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    // Mayor is player-2 (citizen), boss is player-0 nominated as commissioner
    ...
    // Now mayor discards, commissioner (the boss) gets 2 cards
  ...
  it('Boss elected as commissioner at exactly 3 bad policies — mob wins immediately', () => {
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    players[0].isMayor = false;
    players[2].isMayor = true;
...
describe('Adversarial: Auto-enact clears term limits', () => {
  it('Previously term-limited players are eligible after auto-enact', () => {
    // Setup: player-1 was last commissioner (term-limited), player-2 was last mayor
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    players[3].isMayor = true;
    players[0].isMayor = false;
    players[1].wasLastCommissioner = true;
    ...
    // The previously term-limited player-1 (wasLastCommissioner) should now be eligible
  ...
  it('Full sequence: elect → set term limits → 3 fails → auto-enact → nominate ex-commissioner', () => {
    ...
    // Pass election — sets player-2 as wasLastMayor, player-3 as wasLastCommissioner
    state = unanimousVote(state, 'approve');
    expect(state.phase).toBe('policy-session');
    ...
    // player-3 (was last commissioner) should now be eligible for nomination
...
describe('Adversarial: Special election term limits', () => {
  it('Special nomination itself does not set term limits — only the subsequent election does', () => {
    ...
    // Before: player-1 is wasLastMayor, player-2 is wasLastCommissioner
    ...
    players[2].wasLastCommissioner = true;
    ...
    expect(state.players.find(p => p.id === 'player-2')!.wasLastCommissioner).toBe(true);
  });

  it('Special-elected mayor who passes election sets NEW term limits on that pair', () => {
    // player-3 is special-elected mayor, nominates player-4 as commissioner
    const players = makePlayers(7, {
      0: 'mob-boss',
      1: 'mob-soldier',
      2: 'mob-soldier',
    ...
    // After passed election, player-3 should be wasLastMayor, player-4 wasLastCommissioner
...
describe('Adversarial: Execution of term-limited player', () => {
  it('Executing wasLastCommissioner player does not transfer term limit to anyone else', () => {
    const players = makePlayers(7, {
      0: 'mob-boss',
      1: 'mob-soldier',
      2: 'mob-soldier',
    });
    // player-3 was last commissioner (term-limited) and gets executed
    players[3].wasLastCommissioner = true;
    ...
    // No other player should have wasLastCommissioner set (the dead player's flag is irrelevant)
    const alivePlayersWithCommissionerFlag = state.players.filter(
      p => p.isAlive && p.wasLastCommissioner,
    );
    expect(alivePlayersWithCommissionerFlag).toHaveLength(0);
  ...
  it('Dead player is excluded from eligible nominees regardless of term limit status', () => {
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    // Kill player-2, make them wasLastCommissioner (they died while term-limited)
...
// ── 6. Commissioner-cleared event specifics ───────────────────────────────

describe('Adversarial: Commissioner-cleared event', () => {
  ...
  it('(d) Fires even when commissioner is a mob soldier (soldier is not the boss)', () => {
    // player-1 is mob-soldier, elected as commissioner at 3+ bad
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    players[0].isMayor = false;
    players[2].isMayor = true;
    ...
    let state = createTestGameState({
      ...
      subPhase: 'election-voting',
      badPoliciesEnacted: 3,
      nominatedCommissionerId: 'player-1', // mob soldier
      mayorIndex: 2,
      players,
    });
    ...
    // Should fire commissioner-cleared (everyone now knows player-1 is not the boss)
    expect(state.events).toContainEqual(
      expect.objectContaining({ type: 'commissioner-cleared', commissionerId: 'player-1' }),
    );
  });
});
...
describe('Adversarial: Veto state across rounds', () => {
  ...
  it('After rejected veto and commissioner enacts, next round has vetoProposed=false', () => {
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    players[0].isMayor = false;
    players[2].isMayor = true;
    ...
    // Commissioner must now enact — enact the good policy (discard bad)
    state = dispatch(state, { type: 'commissioner-discard', cardIndex: 0 });
    state = advanceDisplayIfNeeded(state);
  ...
  it('Veto cannot be proposed twice in same session after rejection', () => {
    ...
    // Commissioner tries to propose veto again — should fail
    expect(() =>
...
describe('Adversarial: 5-player endgame (3 alive)', () => {
  ...
  it('Term limits at 5 alive: only commissioner is term-limited (mayor can be nominated)', () => {
    // SH rule: at 5 alive, only previous commissioner is term-limited
    const players = makePlayers(5, { 0: 'mob-boss', 1: 'mob-soldier' });
    players[1].isAlive = false;
    players[2].isAlive = false;
    ...
    players[4].wasLastCommissioner = true;
    players[0].isMayor = false;
    ...
    // player-4 is wasLastCommissioner → term-limited (always)
    // player-0 is the only eligible nominee
    // Wait — at 5 alive players (not 5 total players), mayor IS eligible?
    // Actually the rules say: "In games with 5 alive players, only the previous Commissioner is term-limited"
    ...
    // With 3 alive: player-3 is mayor (excluded), player-4 is wasLastCommissioner (excluded),
